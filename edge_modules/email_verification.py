...
def password_reset_url(token: str) -> str:
    return f"{email_verification_base_url()}/reset-password?token={quote(str(token))}"
...
def send_password_reset_email(email: str, reset_url: str):
    ...
    from_email = str(os.getenv("EMAIL_FROM", "no-reply@alexhartel.com")).strip()
    from_name = str(os.getenv("EMAIL_FROM_NAME", "AlexHartel AI Platform")).strip()
    subject = str(os.getenv("PASSWORD_RESET_SUBJECT", "Reset your password")).strip()

    result = {
        "sent": False,
        "delivery": "disabled",
        "debug_reset_url": reset_url if email_verification_debug_return_url() else None,
    }

    if not smtp_enabled:
        print(f"[password-reset] SMTP disabled. Reset {email}: {reset_url}")
        result["delivery"] = "log_only"
        return result
    ...
    if not host or not username or not password:
        print(f"[password-reset] SMTP config incomplete. Reset {email}: {reset_url}")
        result["delivery"] = "smtp_config_missing"
        return result
