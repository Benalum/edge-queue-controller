# Phase 14J-EB - Direct Proxmox sshd Target Candidate Read-Only Diagnostics
...
This phase records the approved read-only diagnostics for a direct Proxmox sshd target candidate.
...
- START_HEAD=931ef50
- START_TAG=controller-phase-14j-ea-direct-proxmox-sshd-target-candidate-plan-2026-06-16
...
The one-time local shell variable candidate was accepted as present, but its parsed host hash matched the already configured Tailscale SSH endpoint host hash.
...
A different candidate is still required. The next candidate must not resolve to the same redacted host hash as the configured Tailscale SSH endpoint. It should be a direct Proxmox OpenSSH daemon target reachable by the controller, such as a LAN/VPN/Tailscale IP/path that reaches Proxmox sshd rather than Tailscale SSH.
...
The next phase should be docs/smoke-only guidance for selecting a different direct Proxmox sshd candidate without exposing raw targets in chat. It should not call controller power endpoints, start workers, mutate DB/jobs, call CT101, call model/Ollama, restart services, mutate service environment, execute Proxmox remote commands, or activate scheduler/primary filtering.

## Boundaries preserved by EB
...
NEXT_SAFE_PHASE=direct_proxmox_sshd_candidate_selection_guidance
