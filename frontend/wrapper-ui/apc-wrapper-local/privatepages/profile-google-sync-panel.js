...
(function apcProfileGoogleSyncPanelStage17kZr7cAppdata() {
  ...
  const signedInPrivateOnlyMarker = 'PROFILE_GOOGLE_SYNC_SIGNED_IN_PRIVATE_PROFILE_ONLY_R11Y';
  ...
  function renderPanel() {
    if (!document.body || !isProfileSurface() || !hasSignedInPrivateProfile()) { removePanel(); return; }
    ...
    panel.innerHTML = [
      ...
      '<button type="button" data-apc-google-sync-bootstrap-appdata>Create hidden Buddies Who Study local data database</button>',
      '<button type="button" data-apc-google-sync-list-appdata>Read Buddies Who Study local data metadata</button>',
      '<button type="button" data-apc-google-sync-rollback-appdata-proof>Rollback/delete Buddies Who Study local data proof files</button>',
      '</div>',
  ...
  document.addEventListener('apc-auth-changed', install);
})();
