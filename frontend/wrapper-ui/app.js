...
// LOGGED_IN_FULL_APP_HANDOFF_V1
// The apex domain stays always-on and public. Logged-in users should use the
// full CT 101 app for protected tools.
const FULL_APP_ROUTES = {
  "/study": "https://companion.alexhartel.com/study",
  "/companion": "https://companion.alexhartel.com/companion",
  "/calendar": "https://calendar.alexhartel.com/calendar",
  "/profile": "https://profile.alexhartel.com/profile",
};

function isFullAppRoute(path) {
  return Boolean(FULL_APP_ROUTES[path]);
}

function fullAppUrlFor(path) {
  return FULL_APP_ROUTES[path] || null;
}

function maybeOpenFullAppForLoggedIn(path) {
  const url = fullAppUrlFor(path);
  if (!url) return false;

  // If not logged in, keep showing the public summary page.
...
// Capture protected nav clicks before the wrapper SPA handles them.
document.addEventListener(
  ...
  (event) => {
    ...
    const path = link.getAttribute("data-route");
    if (!isFullAppRoute(path)) return;

    if (authState?.token) {
      event.preventDefault();
      event.stopPropagation();
      maybeOpenFullAppForLoggedIn(path);
    }
  },
  true
);

// Direct browser loads like alexhartel.com/study while logged in should also
// hand off to the full app. Logged-out users still see public summaries.
setTimeout(() => {
  maybeOpenFullAppForLoggedIn(location.pathname);
}, 500);
