...
echo "=== Stage 6S Universal Intent Router Study route baseline smoke ==="

fail=0

baseline="docs/generated/stage-6s-study-route-baseline.json"
doc="docs/stage-6s-universal-intent-router-study-route-baseline.md"
...
data = json.loads(Path("docs/generated/stage-6s-study-route-baseline.json").read_text())

assert data["stage"] == "6S"
assert data["runtime_behavior_change"] is False

routes = {item["route"] for item in data["study_routes"]}
...
assert shadow["helper_name"] == "_stage6q_study_adapter_shadow"

print("OK: Stage 6S baseline JSON is valid")
PY

python3 -m py_compile edge_intent_router.py edge_controller.py

echo
echo "=== verify Study routes are still classified ==="
for route in \
...
echo
echo "=== verify shadow adapter is still not wired into runtime ==="
...
echo
echo "=== optional live Study route probe, non-mutating baseline only ==="
if systemctl is-active edge-queue-controller >/dev/null 2>&1; then
  ...
  do
    code="$(
      curl -sS -o "/tmp/stage6s$(echo "$route" | tr '/' '_').json" \
        -w "%{http_code}" \
        -X POST "http://127.0.0.1:7070$route" \
        -H 'Content-Type: application/json' \
        --data '{}' || true
    )"
    echo "study_route_probe route=$route http_code=$code"
  done
else
  echo "WARN: controller inactive; skipped optional live Study route probe"
fi
...
if systemctl is-active edge-queue-controller >/dev/null 2>&1; then
  code="$(
    curl -sS -o /tmp/stage6s-router-disabled.json \
      -w "%{http_code}" \
  ...
  if [ "$code" = "404" ]; then
    ...
    cat /tmp/stage6s-router-disabled.json || true
    fail=1
  fi
else
...
if [ "$fail" -eq 0 ]; then
  echo "PASS: Stage 6S Universal Intent Router Study route baseline smoke passed"
else
  echo "FAIL: Stage 6S Universal Intent Router Study route baseline smoke failed"
fi
