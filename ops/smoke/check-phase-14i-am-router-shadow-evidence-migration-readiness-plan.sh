...
PHASE="phase-14i-am-router-shadow-evidence-migration-readiness-plan"
DOC="docs/${PHASE}.md"
SMOKE="ops/smoke/check-${PHASE}.sh"

echo "=== Phase 14I-AM router shadow evidence migration readiness plan ==="
...
echo
echo "=== migration readiness documentation markers ==="
python3 - <<'PY'
from pathlib import Path

doc = Path("docs/phase-14i-am-router-shadow-evidence-migration-readiness-plan.md").read_text()
...
blocked = [
    "CREATE TABLE queued_chat_router_shadow_evidence",
    "INSERT INTO queued_chat_router_shadow_evidence",
    "ALTER TABLE queued_chat_router_shadow_evidence",
]

found = [marker for marker in blocked if marker in doc]
if found:
    raise SystemExit("FAIL: doc contains concrete migration SQL instead of readiness plan: " + ", ".join(found))

print("PASS: migration readiness documentation markers verified")
PY
...
echo
echo "=== no migration files added in this phase ==="
python3 - <<'PY'
from pathlib import Path
import subprocess

expected = {
    "docs/phase-14i-am-router-shadow-evidence-migration-readiness-plan.md",
    "ops/smoke/check-phase-14i-am-router-shadow-evidence-migration-readiness-plan.sh",
}
...
for line in status.splitlines():
    ...
    if (
        ...
        and not path.startswith("ops/smoke/")
    ):
        unexpected.append(line)
...
print("PASS: changed files are limited to Phase 14I-AM docs/smoke when uncommitted, or clean after commit")
PY
...
smoke = Path("ops/smoke/check-phase-14i-am-router-shadow-evidence-migration-readiness-plan.sh").read_text()
...
echo
echo "=== done: Phase 14I-AM router shadow evidence migration readiness smoke complete ==="
