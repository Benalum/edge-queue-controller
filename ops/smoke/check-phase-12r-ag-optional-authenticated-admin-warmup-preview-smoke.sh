...
PHASE="phase-12r-ag-optional-authenticated-admin-warmup-preview-smoke"
fail=0

echo "=== ${PHASE}: optional authenticated live preview checks ==="
...
echo
echo "=== service health ==="
curl -sS --max-time 5 -o /tmp/phase12rag-health.json \
...
unauth_code="$(curl -sS --max-time 8 \
  ...
  -o /tmp/phase12rag-unauth-post.json \
  -w "%{http_code}" \
...
data = json.loads(Path("/tmp/phase12rag-unauth-post.json").read_text())
...
echo
echo "=== optional authenticated admin POST preview check ==="
if [ -z "${EDGE_TEST_ADMIN_BEARER_TOKEN:-}" ]; then
  echo "CHECK: EDGE_TEST_ADMIN_BEARER_TOKEN is not set"
  echo "CHECK: skipping authenticated admin future-preview check"
else
  python3 - <<'PY' || fail=1
import json
import os
...
req = urllib.request.Request(
    ...
    headers={
        ...
        "User-Agent": "phase-12r-ag-optional-admin-preview-check/1.0",
    },
)
...
print("PASS: authenticated admin disabled refusal includes non-executable future preview")
PY
fi
