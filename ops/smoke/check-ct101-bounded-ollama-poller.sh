...
echo "=== CT101 bounded Ollama poller smoke ==="
...
if not models:
    raise SystemExit('FAIL: no Ollama models available on CT101')
...
if command -v tailscale >/dev/null 2>&1; then
  LAPTOP_HOST="${LAPTOP_QUEUE_BOUNDED_OLLAMA_HOST:-$(tailscale ip -4 | head -1)}"
else
  LAPTOP_HOST="${LAPTOP_QUEUE_BOUNDED_OLLAMA_HOST:-$(hostname -I | awk '{print $1}')}"
fi
...
LOG_FILE="/tmp/ct101-bounded-ollama-smoke-$PORT.log"
...
if parsed.get("ok") is True and parsed.get("leftover_count") == 0:
    print("OK: cleanup removed synthetic Stage 5E-21 rows")
PY
...
TOKEN="$TOKEN" LOCAL_BASE_URL="$LOCAL_BASE_URL" IDS_FILE="$IDS_FILE" OLLAMA_MODEL="$OLLAMA_MODEL" python3 - <<'PY'
import json
import os
import time
import urllib.error
...
suffix = f"s5e21-{int(time.time())}-{os.getpid()}"
...
client.psql_run(
    ...
    SET payload_json = jsonb_build_object(
        ...
        requested_model = {json.dumps(ollama_model)!r},
        updated_at = now()
    WHERE id = {json.dumps(job_ok_id)!r};
    """
)

print("OK: laptop API created synthetic Ollama job and prompt payload")
PY
...
LAPTOP_QUEUE_WORKER_NAME="Stage 5E-21 Bounded Ollama Poller" \
LAPTOP_QUEUE_WORKER_NODE_NAME="Stage 5E-21 Synthetic Node" \
...
if not raw:
    raise SystemExit("FAIL: Ollama job row missing")

job = json.loads(raw)

if job["status"] != "complete":
    raise SystemExit(f"FAIL: Ollama job did not complete: {job}")

result = job.get("result_json") or {}

for key in ["reply", "model", "worker", "mode", "elapsed_seconds", "source"]:
    if key not in result:
        raise SystemExit(f"FAIL: missing result_json.{key}: {result}")

if result["source"] != "ct101_bounded_ollama_poller":
    raise SystemExit(f"FAIL: unexpected source: {result}")

if result["mode"] != "bounded_ollama_poller":
    raise SystemExit(f"FAIL: unexpected mode: {result}")

if not str(result["reply"]).strip():
    raise SystemExit(f"FAIL: empty reply: {result}")
...
print("OK: bounded Ollama poller completed synthetic job and returned worker idle")
PY
...
echo "PASS: CT101 bounded Ollama poller smoke passed"
