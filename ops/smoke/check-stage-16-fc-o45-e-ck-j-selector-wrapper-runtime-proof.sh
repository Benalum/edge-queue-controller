...
DOC="docs/stage-16-fc-o45-e-ck-i-selector-wrapper-runtime-proof.md"

test -f "$DOC"

grep -Fq "Selector Wrapper Runtime Proof" "$DOC"
...
grep -Fq "580" "$DOC"
grep -Fq "FC-O45-E-CK-I-SELECTOR-RUNTIME-OK" "$DOC"
...
grep -Fq "edge-deterministic-companion-worker-once@580.service" "$DOC"
grep -Fq "CK_I_SELECTOR_CANDIDATE_COUNT=1" "$DOC"
grep -Fq "marker_selected_wrapper_job_id=580" "$DOC"
...
grep -Fq "operator-safe execution path" "$DOC"

echo "PASS stage-16-fc-o45-e-ck-j selector wrapper runtime proof record smoke"
