...
REPORT="docs/generated/stage-9t-persistent-rollout-activation-control-plane-plan.md"
SMOKE="ops/smoke/check-stage-9t-persistent-rollout-activation-control-plane-plan.sh"
...
LIVE_APP="/tmp/stage9t-live-app.js"
LIVE_STUB="/tmp/stage9t-live-router-shadow-read-stub.js"

echo "=== Stage 9T smoke: persistent rollout activation control-plane plan only ==="

echo
echo "=== report/script checks ==="
[ -f "$REPORT" ] && pass "Stage 9T report exists" || check_fail "Stage 9T report missing"
[ -x "$SMOKE" ] && pass "Stage 9T smoke script is executable" || check_fail "Stage 9T smoke script missing or not executable"
...
for needle in \
  ...
  "Add a read-only status endpoint for persistent rollout state." \
  "Add an admin/operator-only state mutation path later, not in the first implementation." \
...
echo
echo "=== Stage 9R/9S evidence validation ==="
...
live_app_code="$(curl -sS --max-time 8 -o "$LIVE_APP" -w "%{http_code}" "$FRONTEND_BASE/app.js" 2>/tmp/stage9t-live-app.err || printf 'curl_failed')"
live_stub_code="$(curl -sS --max-time 8 -o "$LIVE_STUB" -w "%{http_code}" "$FRONTEND_BASE/router_shadow_read_stub.js" 2>/tmp/stage9t-live-stub.err || printf 'curl_failed')"
...
health_code="$(curl -sS --max-time 5 -o /tmp/stage9t-health.out -w "%{http_code}" "$BASE/health" 2>/tmp/stage9t-health.err || printf 'curl_failed')"
...
post_code="$(curl -sS --max-time 5 -X POST \
  ...
  -d '{"text":"stage9t control plane plan checkpoint","source":"stage9t","surface":"backend-only"}' \
  -o /tmp/stage9t-router-post.out \
  -w "%{http_code}" \
  "$BASE/api/router/dry-run" 2>/tmp/stage9t-router-post.err || printf 'curl_failed')"
...
queue_code="$(curl -sS --max-time 5 -o /tmp/stage9t-system-status.json -w "%{http_code}" "$STATUS_URL" 2>/tmp/stage9t-system-status.err || printf 'curl_failed')"
echo "queue_status_code=$queue_code"

if [ "$queue_code" = "200" ]; then
  python3 - /tmp/stage9t-system-status.json <<'PY'
import json
import sys
...
echo
echo "=== Stage 9T smoke result ==="
if [ "$fail" = "0" ]; then
  echo "PASS: Stage 9T persistent rollout activation control-plane plan verified without enablement"
else
  echo "FAIL: Stage 9T persistent rollout activation control-plane plan found issues"
fi
