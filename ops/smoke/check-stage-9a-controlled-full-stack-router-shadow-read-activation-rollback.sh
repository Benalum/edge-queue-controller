#!/usr/bin/env bash
set -u

fail=0
rollback_needed=0

pass() {
  echo "PASS: $1"
}

check_fail() {
  echo "CHECK: $1"
  fail=1
}

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." 2>/dev/null && pwd)"
if [ -z "${ROOT:-}" ] || [ ! -d "$ROOT" ]; then
  check_fail "could not resolve repo root"
  ROOT="."
fi

cd "$ROOT" || check_fail "could not cd into repo root"

REPORT="docs/generated/stage-9a-controlled-full-stack-router-shadow-read-activation-rollback.md"
EVIDENCE="docs/generated/stage-9a-controlled-full-stack-router-shadow-read-activation-rollback-evidence.json"
SMOKE="ops/smoke/check-stage-9a-controlled-full-stack-router-shadow-read-activation-rollback.sh"

STAGE8Z_REPORT="docs/generated/stage-8z-full-stack-shadow-read-activation-preflight-go-no-go.md"
STAGE8X_EVIDENCE="docs/generated/stage-8x-live-served-frontend-router-shadow-read-disabled-boundary-evidence.json"

APP_JS="frontend/wrapper-ui/app.js"
STUB="frontend/wrapper-ui/router_shadow_read_stub.js"

BASE="http://127.0.0.1:7070"
FRONTEND_BASE="http://127.0.0.1:8787"
STATUS_URL="http://127.0.0.1:8787/api/system/status"

DROPIN_DIR="/etc/systemd/system/edge-queue-controller.service.d"
DROPIN="$DROPIN_DIR/85-stage9a-router-dry-run.conf"

LIVE_STUB="/tmp/stage9a-live-router-shadow-read-stub.js"
LIVE_APP="/tmp/stage9a-live-app.js"
FRONTEND_RESULT="/tmp/stage9a-controlled-shadow-read-result.json"

export STAGE9A_FINAL_RESULT="unknown"
export STAGE9A_HEALTH_BEFORE="unknown"
export STAGE9A_POST_BEFORE="unknown"
export STAGE9A_HEALTH_ENABLED="unknown"
export STAGE9A_POST_ENABLED="unknown"
export STAGE9A_FRONTEND_REQUESTS="unknown"
export STAGE9A_FRONTEND_STATUS="unknown"
export STAGE9A_FRONTEND_OK="unknown"
export STAGE9A_DISPATCH_PERFORMED="unknown"
export STAGE9A_QUEUE_BEFORE_CLEAN="unknown"
export STAGE9A_QUEUE_ENABLED_CLEAN="unknown"
export STAGE9A_QUEUE_AFTER_CLEAN="unknown"
export STAGE9A_HEALTH_AFTER="unknown"
export STAGE9A_POST_AFTER="unknown"
export STAGE9A_ROLLBACK_ENV_ABSENT="unknown"

write_evidence() {
  python3 - "$EVIDENCE" <<'PY'
import json
import os
import sys
from datetime import datetime, timezone
from pathlib import Path

path = Path(sys.argv[1])
data = {
    "stage": "9A",
    "generated_at": datetime.now(timezone.utc).isoformat(),
    "purpose": "controlled full-stack router shadow-read activation and rollback",
    "health_before": os.environ.get("STAGE9A_HEALTH_BEFORE"),
    "post_before": os.environ.get("STAGE9A_POST_BEFORE"),
    "health_enabled": os.environ.get("STAGE9A_HEALTH_ENABLED"),
    "post_enabled": os.environ.get("STAGE9A_POST_ENABLED"),
    "frontend_requests": os.environ.get("STAGE9A_FRONTEND_REQUESTS"),
    "frontend_status": os.environ.get("STAGE9A_FRONTEND_STATUS"),
    "frontend_ok": os.environ.get("STAGE9A_FRONTEND_OK"),
    "dispatch_performed": os.environ.get("STAGE9A_DISPATCH_PERFORMED"),
    "queue_before_clean": os.environ.get("STAGE9A_QUEUE_BEFORE_CLEAN"),
    "queue_enabled_clean": os.environ.get("STAGE9A_QUEUE_ENABLED_CLEAN"),
    "queue_after_clean": os.environ.get("STAGE9A_QUEUE_AFTER_CLEAN"),
    "health_after": os.environ.get("STAGE9A_HEALTH_AFTER"),
    "post_after": os.environ.get("STAGE9A_POST_AFTER"),
    "rollback_env_absent": os.environ.get("STAGE9A_ROLLBACK_ENV_ABSENT"),
    "final_result": os.environ.get("STAGE9A_FINAL_RESULT"),
}
path.parent.mkdir(parents=True, exist_ok=True)
path.write_text(json.dumps(data, indent=2, sort_keys=True) + "\n", encoding="utf-8")
print(f"wrote evidence: {path}")
PY
}

wait_for_health() {
  label="$1"
  tries=30
  code="000"
  while [ "$tries" -gt 0 ]; do
    code="$(curl -sS --max-time 3 -o "/tmp/stage9a-health-${label}.out" -w "%{http_code}" "$BASE/health" 2>"/tmp/stage9a-health-${label}.err" || printf 'curl_failed')"
    if [ "$code" = "200" ]; then
      echo "$code"
      return 0
    fi
    tries=$((tries - 1))
    sleep 1
  done
  echo "$code"
  return 1
}

queue_clean_check() {
  label="$1"
  out="/tmp/stage9a-system-status-${label}.json"
  err="/tmp/stage9a-system-status-${label}.err"

  queue_code="$(curl -sS --max-time 5 -o "$out" -w "%{http_code}" "$STATUS_URL" 2>"$err" || printf 'curl_failed')"
  echo "queue_status_${label}_code=$queue_code"

  if [ "$queue_code" != "200" ]; then
    return 3
  fi

  python3 - "$out" <<'PY'
import json
import sys

path = sys.argv[1]
with open(path, "r", encoding="utf-8") as f:
    data = json.load(f)

matches = []

def walk(value, label="$"):
    if isinstance(value, dict):
        if all(k in value for k in ("queued", "running", "failed")):
            matches.append((label, value.get("queued"), value.get("running"), value.get("failed")))
        for k, v in value.items():
            walk(v, f"{label}.{k}")
    elif isinstance(value, list):
        for i, v in enumerate(value):
            walk(v, f"{label}[{i}]")

walk(data)

if not matches:
    print("MISSING_QUEUE_TRIPLE")
    sys.exit(3)

for label, queued, running, failed in matches:
    print(f"{label}: queued={queued} running={running} failed={failed}")

clean = any(str(q) == "0" and str(r) == "0" and str(f) == "0" for _, q, r, f in matches)
sys.exit(0 if clean else 2)
PY
}

rollback_backend() {
  if [ "$rollback_needed" = "1" ]; then
    echo
    echo "=== rollback: remove Stage 9A backend dry-run drop-in and restart controller ==="
    sudo rm -f "$DROPIN" || true
    sudo systemctl daemon-reload || true
    sudo systemctl restart edge-queue-controller || true

    health_after="$(wait_for_health after-rollback || true)"
    STAGE9A_HEALTH_AFTER="$health_after"
    export STAGE9A_HEALTH_AFTER
    echo "health_after=$health_after"

    controller_env_after="$(systemctl show edge-queue-controller -p Environment --value 2>/dev/null || true)"
    if printf '%s\n' "$controller_env_after" | tr ' ' '\n' | grep -qx 'EDGE_UNIVERSAL_INTENT_ROUTER_DRY_RUN_ENABLED=1'; then
      STAGE9A_ROLLBACK_ENV_ABSENT="false"
      export STAGE9A_ROLLBACK_ENV_ABSENT
      check_fail "rollback failed: backend dry-run env still enabled"
    else
      STAGE9A_ROLLBACK_ENV_ABSENT="true"
      export STAGE9A_ROLLBACK_ENV_ABSENT
      pass "backend dry-run env absent after rollback"
    fi

    post_after="$(curl -sS --max-time 5 -X POST \
      -H 'Content-Type: application/json' \
      -d '{"text":"stage9a rollback check","source":"stage9a","surface":"backend-only"}' \
      -o /tmp/stage9a-router-post-after.out \
      -w "%{http_code}" \
      "$BASE/api/router/dry-run" 2>/tmp/stage9a-router-post-after.err || printf 'curl_failed')"
    STAGE9A_POST_AFTER="$post_after"
    export STAGE9A_POST_AFTER
    echo "post_after=$post_after"

    if [ "$post_after" = "404" ]; then
      pass "POST /api/router/dry-run returned HTTP 404 after rollback"
    else
      check_fail "POST /api/router/dry-run did not return HTTP 404 after rollback"
    fi

    if queue_clean_check after; then
      STAGE9A_QUEUE_AFTER_CLEAN="true"
      export STAGE9A_QUEUE_AFTER_CLEAN
      pass "queue clean after rollback"
    else
      STAGE9A_QUEUE_AFTER_CLEAN="false"
      export STAGE9A_QUEUE_AFTER_CLEAN
      check_fail "queue not clean after rollback"
    fi

    rollback_needed=0
  fi
}

trap 'rollback_backend; STAGE9A_FINAL_RESULT="fail"; export STAGE9A_FINAL_RESULT; write_evidence' INT TERM

echo "=== Stage 9A smoke: controlled full-stack router shadow-read activation and rollback ==="

echo
echo "=== report/script checks ==="
[ -f "$REPORT" ] && pass "Stage 9A report exists" || check_fail "Stage 9A report missing"
[ -x "$SMOKE" ] && pass "Stage 9A smoke script is executable" || check_fail "Stage 9A smoke script missing or not executable"

for needle in \
  "Stage 9A performs the first controlled full-stack router shadow-read activation and rollback." \
  "Set EDGE_UNIVERSAL_INTENT_ROUTER_DRY_RUN_ENABLED=1 only during the controlled test window." \
  "Temporarily set frontend shadow-read flags to true inside the Node VM only." \
  "Send exactly one controlled router shadow-read request." \
  "Verify the response is non-dispatching." \
  "POST /api/router/dry-run returns HTTP 404." \
  "Leave POST /api/router/dry-run enabled after rollback."
do
  if grep -Fq "$needle" "$REPORT" 2>/dev/null; then
    pass "report contains: $needle"
  else
    check_fail "report missing required text: $needle"
  fi
done

echo
echo "=== repo cleanliness check allowing only Stage 9A files ==="
dirty="$(git status --short)"
unexpected="$(printf '%s\n' "$dirty" | grep -vE '^\?\? docs/generated/stage-9a-controlled-full-stack-router-shadow-read-activation-rollback(\.md|-evidence\.json)$|^\?\? ops/smoke/check-stage-9a-controlled-full-stack-router-shadow-read-activation-rollback\.sh$|^$' || true)"
if [ -z "$unexpected" ]; then
  pass "repo has no unexpected dirty files"
else
  check_fail "repo has unexpected dirty files"
  printf '%s\n' "$unexpected"
fi

echo
echo "=== Stage 8Z ancestry/evidence checks ==="
if git log --oneline -14 | grep -q "checkpoint shadow-read activation preflight stage 8z"; then
  pass "recent history includes Stage 8Z preflight"
else
  check_fail "recent history does not include Stage 8Z preflight"
fi

[ -f "$STAGE8Z_REPORT" ] && pass "Stage 8Z report exists" || check_fail "missing Stage 8Z report"
[ -f "$STAGE8X_EVIDENCE" ] && pass "Stage 8X evidence exists" || check_fail "missing Stage 8X evidence"

python3 - "$STAGE8X_EVIDENCE" <<'PY'
import json
import sys
from pathlib import Path

path = Path(sys.argv[1])
if not path.exists():
    sys.exit(1)
data = json.loads(path.read_text(encoding="utf-8"))
expected = {
    "final_result": "pass",
    "local_runtime": "pass",
    "live_stub_code": "200",
    "live_runtime": "pass",
    "post_code": "404",
    "queue_clean": "true",
}
bad = []
for key, value in expected.items():
    actual = str(data.get(key))
    print(f"{key}={actual}")
    if actual != value:
        bad.append((key, value, actual))
if bad:
    sys.exit(2)
print("PASS: Stage 8X evidence values confirmed")
PY
if [ "$?" = "0" ]; then
  pass "Stage 8X evidence confirms disabled boundary"
else
  check_fail "Stage 8X evidence validation failed"
fi

echo
echo "=== frontend safety checks before activation ==="
[ -f "$APP_JS" ] && pass "app.js exists" || check_fail "missing $APP_JS"
[ -f "$STUB" ] && pass "router shadow-read stub exists" || check_fail "missing $STUB"

if grep -q "/api/router/dry-run" "$APP_JS" 2>/dev/null; then
  check_fail "app.js must not contain /api/router/dry-run before activation"
  grep -n "/api/router/dry-run" "$APP_JS" 2>/dev/null | sed -n '1,80p'
else
  pass "app.js contains no /api/router/dry-run"
fi

if grep -q "sendRouterDryRunShadowRead" "$APP_JS" 2>/dev/null; then
  check_fail "app.js is wired to sendRouterDryRunShadowRead before activation"
  grep -n "sendRouterDryRunShadowRead" "$APP_JS" 2>/dev/null | sed -n '1,80p'
else
  pass "app.js is not wired to sendRouterDryRunShadowRead"
fi

grep -q 'const ROUTER_DRY_RUN_ENDPOINT = "/api/router/dry-run";' "$STUB" 2>/dev/null \
  && pass "stub contains backend dry-run endpoint boundary" \
  || check_fail "stub missing backend dry-run endpoint boundary"

grep -q "const ROUTER_SHADOW_READ_ENABLED = false;" "$STUB" 2>/dev/null \
  && pass "ROUTER_SHADOW_READ_ENABLED remains false in repo" \
  || check_fail "ROUTER_SHADOW_READ_ENABLED=false marker missing"

grep -q "const ROUTER_SHADOW_READ_FEATURE_FLAG_DEFAULT = false;" "$STUB" 2>/dev/null \
  && pass "ROUTER_SHADOW_READ_FEATURE_FLAG_DEFAULT remains false in repo" \
  || check_fail "ROUTER_SHADOW_READ_FEATURE_FLAG_DEFAULT=false marker missing"

echo
echo "=== live-served frontend pre-activation checks ==="
live_stub_code="$(curl -sS --max-time 8 -o "$LIVE_STUB" -w "%{http_code}" "$FRONTEND_BASE/router_shadow_read_stub.js" 2>/tmp/stage9a-live-stub.err || printf 'curl_failed')"
live_app_code="$(curl -sS --max-time 8 -o "$LIVE_APP" -w "%{http_code}" "$FRONTEND_BASE/app.js" 2>/tmp/stage9a-live-app.err || printf 'curl_failed')"
echo "live_stub_code=$live_stub_code"
echo "live_app_code=$live_app_code"

if [ "$live_app_code" = "200" ]; then
  pass "live-served app.js fetched with HTTP 200"
  if grep -q "/api/router/dry-run" "$LIVE_APP" 2>/dev/null; then
    check_fail "live-served app.js contains /api/router/dry-run"
  else
    pass "live-served app.js contains no /api/router/dry-run"
  fi
  if grep -q "sendRouterDryRunShadowRead" "$LIVE_APP" 2>/dev/null; then
    check_fail "live-served app.js is wired to sendRouterDryRunShadowRead"
  else
    pass "live-served app.js is not wired to sendRouterDryRunShadowRead"
  fi
else
  check_fail "could not fetch live-served app.js"
fi

if [ "$live_stub_code" = "200" ]; then
  pass "live-served router_shadow_read_stub.js fetched with HTTP 200"
  grep -q 'const ROUTER_DRY_RUN_ENDPOINT = "/api/router/dry-run";' "$LIVE_STUB" 2>/dev/null \
    && pass "live-served stub contains backend dry-run endpoint boundary" \
    || check_fail "live-served stub missing endpoint boundary"
  grep -q "const ROUTER_SHADOW_READ_ENABLED = false;" "$LIVE_STUB" 2>/dev/null \
    && pass "live-served ROUTER_SHADOW_READ_ENABLED remains false" \
    || check_fail "live-served ROUTER_SHADOW_READ_ENABLED=false marker missing"
  grep -q "const ROUTER_SHADOW_READ_FEATURE_FLAG_DEFAULT = false;" "$LIVE_STUB" 2>/dev/null \
    && pass "live-served ROUTER_SHADOW_READ_FEATURE_FLAG_DEFAULT remains false" \
    || check_fail "live-served ROUTER_SHADOW_READ_FEATURE_FLAG_DEFAULT=false marker missing"
else
  check_fail "could not fetch live-served router shadow-read stub"
fi

echo
echo "=== pre-activation live backend checks ==="
health_before="$(wait_for_health before || true)"
STAGE9A_HEALTH_BEFORE="$health_before"
export STAGE9A_HEALTH_BEFORE
echo "health_before=$health_before"
[ "$health_before" = "200" ] && pass "health before activation is HTTP 200" || check_fail "health before activation is not HTTP 200"

post_before="$(curl -sS --max-time 5 -X POST \
  -H 'Content-Type: application/json' \
  -d '{"text":"stage9a pre activation check","source":"stage9a","surface":"backend-only"}' \
  -o /tmp/stage9a-router-post-before.out \
  -w "%{http_code}" \
  "$BASE/api/router/dry-run" 2>/tmp/stage9a-router-post-before.err || printf 'curl_failed')"
STAGE9A_POST_BEFORE="$post_before"
export STAGE9A_POST_BEFORE
echo "post_before=$post_before"
[ "$post_before" = "404" ] && pass "POST /api/router/dry-run is disabled before activation" || check_fail "POST /api/router/dry-run not disabled before activation"

controller_env_before="$(systemctl show edge-queue-controller -p Environment --value 2>/dev/null || true)"
if printf '%s\n' "$controller_env_before" | tr ' ' '\n' | grep -qx 'EDGE_UNIVERSAL_INTENT_ROUTER_DRY_RUN_ENABLED=1'; then
  check_fail "backend dry-run env already enabled before activation"
else
  pass "backend dry-run env absent before activation"
fi

if queue_clean_check before; then
  STAGE9A_QUEUE_BEFORE_CLEAN="true"
  export STAGE9A_QUEUE_BEFORE_CLEAN
  pass "queue clean before activation"
else
  STAGE9A_QUEUE_BEFORE_CLEAN="false"
  export STAGE9A_QUEUE_BEFORE_CLEAN
  check_fail "queue not clean before activation"
fi

if [ "$fail" != "0" ]; then
  echo "CHECK: pre-activation checks failed, refusing to activate"
else
  echo
  echo "=== activate backend dry-run using temporary systemd drop-in ==="
  sudo mkdir -p "$DROPIN_DIR" || fail=1
  cat <<'EOF' | sudo tee "$DROPIN" >/dev/null
[Service]
Environment=EDGE_UNIVERSAL_INTENT_ROUTER_DRY_RUN_ENABLED=1
EOF
  sudo systemctl daemon-reload || fail=1
  sudo systemctl restart edge-queue-controller || fail=1
  rollback_needed=1

  health_enabled="$(wait_for_health enabled || true)"
  STAGE9A_HEALTH_ENABLED="$health_enabled"
  export STAGE9A_HEALTH_ENABLED
  echo "health_enabled=$health_enabled"
  [ "$health_enabled" = "200" ] && pass "health after activation is HTTP 200" || check_fail "health after activation is not HTTP 200"

  controller_env_enabled="$(systemctl show edge-queue-controller -p Environment --value 2>/dev/null || true)"
  if printf '%s\n' "$controller_env_enabled" | tr ' ' '\n' | grep -qx 'EDGE_UNIVERSAL_INTENT_ROUTER_DRY_RUN_ENABLED=1'; then
    pass "backend dry-run env present during controlled activation"
  else
    check_fail "backend dry-run env missing during controlled activation"
  fi

  post_enabled="$(curl -sS --max-time 8 -X POST \
    -H 'Content-Type: application/json' \
    -d '{"text":"stage9a backend enabled check","source":"stage9a","surface":"backend-only"}' \
    -o /tmp/stage9a-router-post-enabled.out \
    -w "%{http_code}" \
    "$BASE/api/router/dry-run" 2>/tmp/stage9a-router-post-enabled.err || printf 'curl_failed')"
  STAGE9A_POST_ENABLED="$post_enabled"
  export STAGE9A_POST_ENABLED
  echo "post_enabled=$post_enabled"
  [ "$post_enabled" = "200" ] && pass "POST /api/router/dry-run enabled during activation" || check_fail "POST /api/router/dry-run did not return HTTP 200 during activation"

  echo
  echo "=== exactly one controlled frontend shadow-read request through live-served stub ==="
  if command -v node >/dev/null 2>&1 && [ -f "$LIVE_STUB" ]; then
    STAGE9A_BACKEND_URL="$BASE/api/router/dry-run" \
    STAGE9A_RESULT_FILE="$FRONTEND_RESULT" \
    node <<'NODE'
const fs = require("fs");
const vm = require("vm");

const code = fs.readFileSync("/tmp/stage9a-live-router-shadow-read-stub.js", "utf8");
const backendUrl = process.env.STAGE9A_BACKEND_URL;
const resultFile = process.env.STAGE9A_RESULT_FILE;

let fetchCalls = 0;

const sandbox = {
  console,
  window: {},
  document: {
    addEventListener: () => {},
    removeEventListener: () => {},
    querySelector: () => null,
    querySelectorAll: () => []
  },
  navigator: { userAgent: "stage9a-controlled-frontend-shadow-read" },
  location: { href: "http://127.0.0.1:8787/stage9a-controlled-shadow-read" },
  localStorage: {
    getItem: () => null,
    setItem: () => {},
    removeItem: () => {}
  },
  fetch: async (url, options) => {
    fetchCalls += 1;
    if (fetchCalls > 1) {
      throw new Error("more than one frontend shadow-read fetch attempted");
    }

    const target = String(url);
    if (target !== "/api/router/dry-run") {
      throw new Error("unexpected frontend shadow-read endpoint: " + target);
    }

    const body = options && options.body ? String(options.body) : "";
    const parsed = body ? JSON.parse(body) : {};
    if (parsed.dry_run !== true) {
      throw new Error("frontend request did not set dry_run=true");
    }
    if (parsed.dispatch_requested !== false || parsed.dispatch_performed !== false) {
      throw new Error("frontend request was not explicitly non-dispatching");
    }

    return fetch(backendUrl, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body
    });
  },
  setTimeout,
  clearTimeout
};

vm.createContext(sandbox);
vm.runInContext(code, sandbox, { filename: "stage9a-live-router-shadow-read-stub.js" });

const shadow = sandbox.window.EdgeRouterShadowRead;
if (!shadow) {
  throw new Error("EdgeRouterShadowRead namespace missing");
}
if (shadow.ROUTER_SHADOW_READ_ENABLED !== false) {
  throw new Error("live-served repo flag should start false");
}
if (shadow.ROUTER_SHADOW_READ_FEATURE_FLAG_DEFAULT !== false) {
  throw new Error("live-served feature flag default should start false");
}
if (typeof shadow.sendRouterDryRunShadowRead !== "function") {
  throw new Error("sendRouterDryRunShadowRead missing");
}

shadow.ROUTER_SHADOW_READ_ENABLED = true;
shadow.ROUTER_SHADOW_READ_FEATURE_FLAG_DEFAULT = true;

Promise.resolve(
  shadow.sendRouterDryRunShadowRead({
    text: "stage9a exactly one controlled frontend shadow-read",
    source: "stage9a",
    surface: "node-vm-live-served-stub",
    route_hint: "controlled-shadow-read"
  })
).then((result) => {
  shadow.ROUTER_SHADOW_READ_ENABLED = false;
  shadow.ROUTER_SHADOW_READ_FEATURE_FLAG_DEFAULT = false;

  const body = result && result.body ? result.body : {};
  const output = {
    ok: result ? result.ok === true : false,
    status: result ? result.status : null,
    endpoint: result ? result.endpoint : null,
    requests: fetchCalls,
    dispatch_requested: result ? result.dispatch_requested : null,
    dispatch_performed: result ? result.dispatch_performed : null,
    body_dispatch_performed: body.dispatch_performed,
    body_dispatch_performed_type: typeof body.dispatch_performed,
    body_ok: body.ok,
    body_dry_run: body.dry_run,
    body_keys: body && typeof body === "object" ? Object.keys(body).sort() : []
  };

  fs.writeFileSync(resultFile, JSON.stringify(output, null, 2) + "\n");
  console.log(JSON.stringify(output, null, 2));

  if (fetchCalls !== 1) {
    throw new Error("expected exactly one frontend fetch, got " + fetchCalls);
  }
  if (!output.ok || output.status !== 200) {
    throw new Error("controlled frontend shadow-read did not return HTTP 200");
  }
  if (output.dispatch_performed !== false) {
    throw new Error("wrapper dispatch_performed was not false");
  }
  if (!(body.dispatch_performed === false || body.dispatch_performed === undefined || body.dispatch_performed === null)) {
    throw new Error("body dispatch_performed indicated dispatch");
  }

  console.log("PASS: controlled frontend shadow-read sent exactly one non-dispatching request");
}).catch((error) => {
  try {
    shadow.ROUTER_SHADOW_READ_ENABLED = false;
    shadow.ROUTER_SHADOW_READ_FEATURE_FLAG_DEFAULT = false;
  } catch (_) {}
  console.error("CHECK:", error.message || error);
  process.exit(1);
});
NODE
    frontend_node_check=$?
    if [ "$frontend_node_check" = "0" ]; then
      pass "controlled frontend shadow-read Node VM check passed"
    else
      check_fail "controlled frontend shadow-read Node VM check failed"
    fi
  else
    check_fail "node or live stub unavailable for controlled frontend shadow-read"
  fi

  if [ -f "$FRONTEND_RESULT" ]; then
    cat "$FRONTEND_RESULT"
    frontend_requests="$(python3 -c 'import json; print(json.load(open("'"$FRONTEND_RESULT"'")).get("requests"))' 2>/dev/null || echo unknown)"
    frontend_status="$(python3 -c 'import json; print(json.load(open("'"$FRONTEND_RESULT"'")).get("status"))' 2>/dev/null || echo unknown)"
    frontend_ok="$(python3 -c 'import json; print(json.load(open("'"$FRONTEND_RESULT"'")).get("ok"))' 2>/dev/null || echo unknown)"
    dispatch_performed="$(python3 -c 'import json; print(json.load(open("'"$FRONTEND_RESULT"'")).get("dispatch_performed"))' 2>/dev/null || echo unknown)"
    body_dispatch_performed="$(python3 -c 'import json; print(json.load(open("'"$FRONTEND_RESULT"'")).get("body_dispatch_performed"))' 2>/dev/null || echo unknown)"

    STAGE9A_FRONTEND_REQUESTS="$frontend_requests"
    STAGE9A_FRONTEND_STATUS="$frontend_status"
    STAGE9A_FRONTEND_OK="$frontend_ok"
    STAGE9A_DISPATCH_PERFORMED="$dispatch_performed"
    export STAGE9A_FRONTEND_REQUESTS STAGE9A_FRONTEND_STATUS STAGE9A_FRONTEND_OK STAGE9A_DISPATCH_PERFORMED

    [ "$frontend_requests" = "1" ] && pass "exactly one frontend request was sent" || check_fail "frontend request count was not exactly one"
    [ "$frontend_status" = "200" ] && pass "frontend shadow-read returned HTTP 200" || check_fail "frontend shadow-read did not return HTTP 200"
    [ "$frontend_ok" = "True" ] || [ "$frontend_ok" = "true" ] && pass "frontend shadow-read ok=true" || check_fail "frontend shadow-read ok was not true"

    if [ "$dispatch_performed" = "False" ] || [ "$dispatch_performed" = "false" ]; then
      pass "wrapper dispatch_performed=false"
    else
      check_fail "wrapper dispatch_performed was not false"
    fi

    if [ "$body_dispatch_performed" = "False" ] || [ "$body_dispatch_performed" = "false" ] || [ "$body_dispatch_performed" = "None" ] || [ "$body_dispatch_performed" = "null" ]; then
      pass "backend body did not indicate dispatch"
    else
      check_fail "backend body indicates dispatch_performed=$body_dispatch_performed"
    fi
  else
    check_fail "frontend result file missing"
  fi

  if queue_clean_check enabled; then
    STAGE9A_QUEUE_ENABLED_CLEAN="true"
    export STAGE9A_QUEUE_ENABLED_CLEAN
    pass "queue remained clean after controlled frontend shadow-read"
  else
    STAGE9A_QUEUE_ENABLED_CLEAN="false"
    export STAGE9A_QUEUE_ENABLED_CLEAN
    check_fail "queue not clean after controlled frontend shadow-read"
  fi
fi

echo
echo "=== explicit rollback now ==="
rollback_backend

echo
echo "=== post-rollback frontend file/live safety checks ==="
if grep -q "/api/router/dry-run" "$APP_JS" 2>/dev/null; then
  check_fail "app.js contains /api/router/dry-run after rollback"
else
  pass "app.js still contains no /api/router/dry-run after rollback"
fi

if grep -q "sendRouterDryRunShadowRead" "$APP_JS" 2>/dev/null; then
  check_fail "app.js wired to sendRouterDryRunShadowRead after rollback"
else
  pass "app.js still not wired to sendRouterDryRunShadowRead after rollback"
fi

grep -q "const ROUTER_SHADOW_READ_ENABLED = false;" "$STUB" 2>/dev/null \
  && pass "repo ROUTER_SHADOW_READ_ENABLED remains false after rollback" \
  || check_fail "repo ROUTER_SHADOW_READ_ENABLED=false missing after rollback"

grep -q "const ROUTER_SHADOW_READ_FEATURE_FLAG_DEFAULT = false;" "$STUB" 2>/dev/null \
  && pass "repo ROUTER_SHADOW_READ_FEATURE_FLAG_DEFAULT remains false after rollback" \
  || check_fail "repo ROUTER_SHADOW_READ_FEATURE_FLAG_DEFAULT=false missing after rollback"

echo
echo "=== timer and temporary port checks after rollback ==="
power_timer="$(systemctl is-active edge-queue-power-auto-tick.timer 2>/dev/null || true)"
remediation_timer="$(systemctl is-active edge-queue-remediation-tick.timer 2>/dev/null || true)"
legacy_active="$(systemctl is-active edge-queue-scheduler-tick.timer 2>/dev/null || true)"
legacy_enabled="$(systemctl is-enabled edge-queue-scheduler-tick.timer 2>/dev/null || true)"

echo "edge-queue-power-auto-tick.timer active=$power_timer"
echo "edge-queue-remediation-tick.timer active=$remediation_timer"
echo "edge-queue-scheduler-tick.timer active=$legacy_active enabled=$legacy_enabled"

[ "$power_timer" = "active" ] && pass "modern power auto timer is active" || check_fail "modern power auto timer is not active"
[ "$remediation_timer" = "active" ] && pass "modern remediation timer is active" || check_fail "modern remediation timer is not active"

if [ "$legacy_active" = "inactive" ] || [ "$legacy_active" = "unknown" ]; then
  pass "legacy scheduler timer is not active"
else
  check_fail "legacy scheduler timer is unexpectedly active/state=$legacy_active"
fi

if [ "$legacy_enabled" = "disabled" ] || [ "$legacy_enabled" = "masked" ]; then
  pass "legacy scheduler timer is disabled/masked"
else
  check_fail "legacy scheduler timer is not disabled/masked; enabled_state=$legacy_enabled"
fi

if ss -ltnp 2>/dev/null | grep -q ':7076'; then
  check_fail "port 7076 appears to be listening"
  ss -ltnp | grep ':7076' || true
else
  pass "port 7076 is not listening"
fi

echo
echo "=== write Stage 9A evidence ==="
if [ "$fail" = "0" ]; then
  STAGE9A_FINAL_RESULT="pass"
else
  STAGE9A_FINAL_RESULT="fail"
fi
export STAGE9A_FINAL_RESULT
write_evidence

echo
echo "=== Stage 9A smoke result ==="
if [ "$fail" = "0" ]; then
  echo "PASS: Stage 9A controlled full-stack activation and rollback verified"
else
  echo "FAIL: Stage 9A controlled full-stack activation and rollback found issues"
fi

exit "$fail"
