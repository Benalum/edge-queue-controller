...
REPORT="docs/generated/stage-9w-live-persistent-rollout-status-stability-checkpoint.md"
EVIDENCE="docs/generated/stage-9w-live-persistent-rollout-status-stability-checkpoint-evidence.json"
SMOKE="ops/smoke/check-stage-9w-live-persistent-rollout-status-stability-checkpoint.sh"
...
LIVE_STATUS="/tmp/stage9w-persistent-rollout-status.json"
LIVE_APP="/tmp/stage9w-live-app.js"
LIVE_STUB="/tmp/stage9w-live-router-shadow-read-stub.js"

export STAGE9W_FINAL_RESULT="unknown"
export STAGE9W_HEALTH_CODE="unknown"
export STAGE9W_STATUS_CODE="unknown"
export STAGE9W_STATUS_RUNTIME="unknown"
export STAGE9W_MUTATION_CODE="unknown"
export STAGE9W_POST_CODE="unknown"
export STAGE9W_ENV_ABSENT="unknown"
export STAGE9W_QUEUE_CLEAN="unknown"
...
data = {
    ...
    "purpose": "live persistent rollout status stability checkpoint",
    "health_code": os.environ.get("STAGE9W_HEALTH_CODE"),
    "status_code": os.environ.get("STAGE9W_STATUS_CODE"),
    "status_runtime": os.environ.get("STAGE9W_STATUS_RUNTIME"),
    "mutation_code": os.environ.get("STAGE9W_MUTATION_CODE"),
    "post_code": os.environ.get("STAGE9W_POST_CODE"),
    "env_absent": os.environ.get("STAGE9W_ENV_ABSENT"),
    "queue_clean": os.environ.get("STAGE9W_QUEUE_CLEAN"),
    "final_result": os.environ.get("STAGE9W_FINAL_RESULT"),
}
...
echo "=== Stage 9W smoke: live persistent rollout status stability checkpoint ==="

echo
echo "=== report/script checks ==="
[ -f "$REPORT" ] && pass "Stage 9W report exists" || check_fail "Stage 9W report missing"
[ -x "$SMOKE" ] && pass "Stage 9W smoke script is executable" || check_fail "Stage 9W smoke script missing or not executable"
...
echo
echo "=== Stage 9V evidence validation ==="
[ -f "$STAGE9V_EVIDENCE" ] && pass "Stage 9V evidence exists" || check_fail "Stage 9V evidence missing"

python3 - "$STAGE9V_EVIDENCE" <<'PY'
import json
import sys
from pathlib import Path
...
if bad:
    print("CHECK: Stage 9V evidence did not match expected values")
    for item in bad:
        print(item)
    sys.exit(2)

print("PASS: Stage 9V evidence confirms live disabled status verification")
PY
if [ "$?" = "0" ]; then
  pass "Stage 9V evidence values confirmed"
else
  check_fail "Stage 9V evidence validation failed"
fi
...
health_code="$(curl -sS --max-time 5 -o /tmp/stage9w-health.out -w "%{http_code}" "$BASE/health" 2>/tmp/stage9w-health.err || printf 'curl_failed')"
STAGE9W_HEALTH_CODE="$health_code"
export STAGE9W_HEALTH_CODE
...
echo
echo "=== live read-only persistent rollout status endpoint stability check ==="
status_code="$(curl -sS --max-time 5 -o "$LIVE_STATUS" -w "%{http_code}" "$ROLLOUT_STATUS_URL" 2>/tmp/stage9w-rollout-status.err || printf 'curl_failed')"
STAGE9W_STATUS_CODE="$status_code"
export STAGE9W_STATUS_CODE
...
PY
  ...
  if [ "$status_runtime" = "0" ]; then
    STAGE9W_STATUS_RUNTIME="pass"
    export STAGE9W_STATUS_RUNTIME
    pass "live status response values confirmed"
  else
    STAGE9W_STATUS_RUNTIME="fail"
    export STAGE9W_STATUS_RUNTIME
    check_fail "live status response validation failed"
  fi
else
  STAGE9W_STATUS_RUNTIME="not_run"
  export STAGE9W_STATUS_RUNTIME
  check_fail "GET /api/router/persistent-rollout/status did not return HTTP 200"
fi

echo
echo "=== mutation path remains unavailable ==="
mutation_code="$(curl -sS --max-time 5 -X POST \
  -H 'Content-Type: application/json' \
  -d '{"enabled":true}' \
  -o /tmp/stage9w-rollout-mutation.out \
  -w "%{http_code}" \
  "$ROLLOUT_STATUS_URL" 2>/tmp/stage9w-rollout-mutation.err || printf 'curl_failed')"
STAGE9W_MUTATION_CODE="$mutation_code"
export STAGE9W_MUTATION_CODE
...
post_code="$(curl -sS --max-time 5 -X POST \
  -H 'Content-Type: application/json' \
  -d '{"text":"stage9w status stability checkpoint","source":"stage9w","surface":"backend-only"}' \
  -o /tmp/stage9w-router-post.out \
  -w "%{http_code}" \
  "$BASE/api/router/dry-run" 2>/tmp/stage9w-router-post.err || printf 'curl_failed')"
STAGE9W_POST_CODE="$post_code"
export STAGE9W_POST_CODE
...
if printf '%s\n' "$controller_env" | tr ' ' '\n' | grep -qx 'EDGE_UNIVERSAL_INTENT_ROUTER_DRY_RUN_ENABLED=1'; then
  STAGE9W_ENV_ABSENT="false"
  export STAGE9W_ENV_ABSENT
  check_fail "backend dry-run env is enabled"
else
  STAGE9W_ENV_ABSENT="true"
  export STAGE9W_ENV_ABSENT
...
live_app_code="$(curl -sS --max-time 8 -o "$LIVE_APP" -w "%{http_code}" "$FRONTEND_BASE/app.js" 2>/tmp/stage9w-live-app.err || printf 'curl_failed')"
live_stub_code="$(curl -sS --max-time 8 -o "$LIVE_STUB" -w "%{http_code}" "$FRONTEND_BASE/router_shadow_read_stub.js" 2>/tmp/stage9w-live-stub.err || printf 'curl_failed')"
...
queue_code="$(curl -sS --max-time 5 -o /tmp/stage9w-system-status.json -w "%{http_code}" "$STATUS_URL" 2>/tmp/stage9w-system-status.err || printf 'curl_failed')"
echo "queue_status_code=$queue_code"

if [ "$queue_code" = "200" ]; then
  python3 - /tmp/stage9w-system-status.json <<'PY'
import json
import sys
...
echo
echo "=== write Stage 9W evidence ==="
if [ "$fail" = "0" ]; then
  STAGE9W_FINAL_RESULT="pass"
else
  STAGE9W_FINAL_RESULT="fail"
fi
export STAGE9W_FINAL_RESULT
write_evidence

echo
echo "=== Stage 9W smoke result ==="
if [ "$fail" = "0" ]; then
  echo "PASS: Stage 9W live persistent rollout status stability verified"
else
  echo "FAIL: Stage 9W live persistent rollout status stability found issues"
fi
