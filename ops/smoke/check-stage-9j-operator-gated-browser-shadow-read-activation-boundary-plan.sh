...
REPORT="docs/generated/stage-9j-operator-gated-browser-shadow-read-activation-boundary-plan.md"
SMOKE="ops/smoke/check-stage-9j-operator-gated-browser-shadow-read-activation-boundary-plan.sh"
...
STAGE9I_REPORT="docs/generated/stage-9i-user-facing-browser-shadow-read-activation-decision-checkpoint.md"
...
LIVE_APP="/tmp/stage9j-live-app.js"
LIVE_STUB="/tmp/stage9j-live-router-shadow-read-stub.js"

echo "=== Stage 9J smoke: operator-gated activation boundary plan only ==="

echo
echo "=== report/script checks ==="
[ -f "$REPORT" ] && pass "Stage 9J report exists" || check_fail "Stage 9J report missing"
[ -x "$SMOKE" ] && pass "Stage 9J smoke script is executable" || check_fail "Stage 9J smoke script missing or not executable"
...
echo
echo "=== Stage 9G/9H/9I evidence and report validation ==="
...
[ -f "$STAGE9I_REPORT" ] && pass "Stage 9I report exists" || check_fail "Stage 9I report missing"
...
live_app_code="$(curl -sS --max-time 8 -o "$LIVE_APP" -w "%{http_code}" "$FRONTEND_BASE/app.js" 2>/tmp/stage9j-live-app.err || printf 'curl_failed')"
live_stub_code="$(curl -sS --max-time 8 -o "$LIVE_STUB" -w "%{http_code}" "$FRONTEND_BASE/router_shadow_read_stub.js" 2>/tmp/stage9j-live-stub.err || printf 'curl_failed')"
...
health_code="$(curl -sS --max-time 5 -o /tmp/stage9j-health.out -w "%{http_code}" "$BASE/health" 2>/tmp/stage9j-health.err || printf 'curl_failed')"
...
post_code="$(curl -sS --max-time 5 -X POST \
  ...
  -d '{"text":"stage9j operator gate plan checkpoint","source":"stage9j","surface":"backend-only"}' \
  -o /tmp/stage9j-router-post.out \
  -w "%{http_code}" \
  "$BASE/api/router/dry-run" 2>/tmp/stage9j-router-post.err || printf 'curl_failed')"
...
queue_code="$(curl -sS --max-time 5 -o /tmp/stage9j-system-status.json -w "%{http_code}" "$STATUS_URL" 2>/tmp/stage9j-system-status.err || printf 'curl_failed')"
echo "queue_status_code=$queue_code"

if [ "$queue_code" = "200" ]; then
  python3 - /tmp/stage9j-system-status.json <<'PY'
import json
import sys
...
echo
echo "=== Stage 9J smoke result ==="
if [ "$fail" = "0" ]; then
  echo "PASS: Stage 9J operator-gated browser shadow-read activation boundary plan verified without enablement"
else
  echo "FAIL: Stage 9J operator-gated browser shadow-read activation boundary plan found issues"
fi
