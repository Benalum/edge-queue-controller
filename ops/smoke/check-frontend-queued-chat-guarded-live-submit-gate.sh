...
echo "=== frontend queued chat guarded live-submit gate smoke ==="
...
require_fixed docs/frontend-queued-chat-guarded-live-submit-gate.md "does not change frontend runtime behavior" "no runtime change"
require_fixed docs/frontend-queued-chat-guarded-live-submit-gate.md "does not wire queued chat into the real submit handler" "no submit wiring"
require_fixed docs/frontend-queued-chat-guarded-live-submit-gate.md "guardedLiveSubmitGateWired false" "gate not wired doc"
require_fixed docs/frontend-queued-chat-guarded-live-submit-gate.md "Stage 5F-64 should add a mocked test" "next stage"
...
require_fixed frontend/wrapper-ui/app.js "Stage 5F-57: guarded queued submit branch skeleton marker." "guarded skeleton marker"
...
marker = "Stage 5F-63: guarded live-submit gate marker."
if text.count(marker) != 1:
    print(f"FAIL: expected exactly one Stage 5F-63 gate marker, found {text.count(marker)}")
    sys.exit(1)
...
nearby = "\n".join(lines[marker_line - 1:min(len(lines), marker_line + 160)])
anchors = [
...
print("OK: Stage 5F-63 marker is near live submit path")
print("OK: guarded live-submit gate is not called from live submit")
...
echo "PASS: frontend queued chat guarded live-submit gate smoke passed"
