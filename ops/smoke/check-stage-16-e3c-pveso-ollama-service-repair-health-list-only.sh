...
{
echo "=== Stage 16 E3C smoke: PVESO Ollama health/list only ==="
...
for spec in "/:200" "/login:200" "/api/me:401" "/api/system/status:200" "/system/status:200"; do
  path="${spec%:*}"
  expect="${spec##*:}"
  code="$(curl -sS -o /tmp/apc_e3c_smoke_public_body.txt -w '%{http_code}' --max-time 20 "${DOMAIN}${path}" 2>/tmp/apc_e3c_smoke_public_err.txt || true)"
  echo "public_path=$path http=${code:-<curl_failed>} expect=$expect"
  [ "$code" = "$expect" ] || mark_fail "public route mismatch: $path"
done
rm -f /tmp/apc_e3c_smoke_public_body.txt /tmp/apc_e3c_smoke_public_err.txt
...
echo
echo "--- PVESO Ollama health/list only ---"
...
curl -fsS --max-time 20 http://127.0.0.1:11434/api/tags \
  | python3 -c 'import sys,json; data=json.load(sys.stdin); print("api_tags_ok=yes"); print("api_tags_model_count="+str(len(data.get("models",[]))))'
