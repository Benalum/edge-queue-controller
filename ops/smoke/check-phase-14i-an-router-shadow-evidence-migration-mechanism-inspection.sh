...
PHASE="phase-14i-an-router-shadow-evidence-migration-mechanism-inspection"
DOC="docs/${PHASE}.md"
SMOKE="ops/smoke/check-${PHASE}.sh"

echo "=== Phase 14I-AN router shadow evidence migration mechanism inspection ==="
...
echo
echo "=== migration mechanism documentation markers ==="
python3 - <<'PY'
from pathlib import Path

doc = Path("docs/phase-14i-an-router-shadow-evidence-migration-mechanism-inspection.md").read_text()
...
if found:
    raise SystemExit("FAIL: doc contains concrete SQL instead of mechanism inspection: " + ", ".join(found))

print("PASS: migration mechanism documentation markers verified")
PY
...
expected = {
    "docs/phase-14i-an-router-shadow-evidence-migration-mechanism-inspection.md",
    "ops/smoke/check-phase-14i-an-router-shadow-evidence-migration-mechanism-inspection.sh",
}
...
print("PASS: changed files are limited to Phase 14I-AN docs/smoke")
PY
...
smoke = Path("ops/smoke/check-phase-14i-an-router-shadow-evidence-migration-mechanism-inspection.sh").read_text()
...
echo
echo "=== done: Phase 14I-AN router shadow evidence migration mechanism inspection smoke complete ==="
