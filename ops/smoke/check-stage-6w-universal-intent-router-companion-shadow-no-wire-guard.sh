...
echo "=== Stage 6W Universal Intent Router Companion shadow no-wire guard smoke ==="

fail=0

contract="docs/generated/stage-6w-companion-shadow-adapter-no-wire-guard.json"
doc="docs/stage-6w-universal-intent-router-companion-shadow-no-wire-guard.md"
...
data = json.loads(Path("docs/generated/stage-6w-companion-shadow-adapter-no-wire-guard.json").read_text())

assert data["stage"] == "6W"
assert data["runtime_behavior_change"] is False
assert data["helper"] == "_stage6v_companion_adapter_shadow"
...
print("OK: Stage 6W no-wire guard contract is valid")
PY
...
if systemctl is-active edge-queue-controller >/dev/null 2>&1; then
  code="$(
    curl -sS -o /tmp/stage6w-disabled.json \
      -w "%{http_code}" \
  ...
  if [ "$code" = "404" ]; then
    ...
    cat /tmp/stage6w-disabled.json || true
    fail=1
  fi
else
...
if [ "$fail" -eq 0 ]; then
  echo "PASS: Stage 6W Universal Intent Router Companion shadow no-wire guard smoke passed"
else
  echo "FAIL: Stage 6W Universal Intent Router Companion shadow no-wire guard smoke failed"
fi
