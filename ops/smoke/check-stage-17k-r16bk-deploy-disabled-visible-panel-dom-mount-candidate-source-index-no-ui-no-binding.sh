#!/usr/bin/env bash
set -euo pipefail
stage="stage-17k-r16bk-deploy-disabled-visible-panel-dom-mount-candidate-source-index-no-ui-no-binding"
...
candidate="study-card-images-disabled-visible-panel-dom-mount-candidate.js?v=stage17k-r16bj-load-disabled-visible-panel-dom-mount-candidate-source-index-source-only-20260708"
line_of() {
  python3 - "$index" "$1" <<'PY'
...
for n in "$visible" "$adapter" "$dom" "$slot" "$mount" "$safe" "$ready" "$candidate"; do
  [ "$(count_of "$n")" = "1" ] || { echo "FAIL expected one index entry for $n" >&2; exit 1; }
done
vl=$(line_of "$visible"); al=$(line_of "$adapter"); dl=$(line_of "$dom"); sl=$(line_of "$slot"); ml=$(line_of "$mount"); sf=$(line_of "$safe"); rl=$(line_of "$ready"); cl=$(line_of "$candidate")
[ "$vl" -lt "$al" ] && [ "$al" -lt "$dl" ] && [ "$dl" -lt "$sl" ] && [ "$sl" -lt "$ml" ] && [ "$ml" -lt "$sf" ] && [ "$sf" -lt "$rl" ] && [ "$rl" -lt "$cl" ] || { echo "FAIL load order" >&2; exit 1; }
grep -q 'APC_STUDY_CARD_IMAGES_DISABLED_VISIBLE_PANEL_DOM_MOUNT_CANDIDATE_R16BI_SOURCE_ONLY' "$root/privatepages/study-card-images-disabled-visible-panel-dom-mount-candidate.js" || { echo "FAIL DOM mount candidate marker missing" >&2; exit 1; }
