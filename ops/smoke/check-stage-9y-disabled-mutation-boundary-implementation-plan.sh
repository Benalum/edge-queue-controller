...
REPORT="docs/generated/stage-9y-disabled-mutation-boundary-implementation-plan.md"
SMOKE="ops/smoke/check-stage-9y-disabled-mutation-boundary-implementation-plan.sh"
...
LIVE_STATUS="/tmp/stage9y-persistent-rollout-status.json"
LIVE_APP="/tmp/stage9y-live-app.js"
LIVE_STUB="/tmp/stage9y-live-router-shadow-read-stub.js"

echo "=== Stage 9Y smoke: disabled mutation-boundary implementation plan only ==="

echo
echo "=== report/script checks ==="
[ -f "$REPORT" ] && pass "Stage 9Y report exists" || check_fail "Stage 9Y report missing"
[ -x "$SMOKE" ] && pass "Stage 9Y smoke script is executable" || check_fail "Stage 9Y smoke script missing or not executable"
...
for needle in \
  ...
  "persistent_rollout_mutation_disabled" \
...
echo
echo "=== Stage 9W evidence and Stage 9X report validation ==="
[ -f "$STAGE9W_EVIDENCE" ] && pass "Stage 9W evidence exists" || check_fail "Stage 9W evidence missing"
[ -f "$STAGE9X_REPORT" ] && pass "Stage 9X report exists" || check_fail "Stage 9X report missing"

python3 - "$STAGE9W_EVIDENCE" <<'PY'
import json
import sys
...
if bad:
    print("CHECK: Stage 9W evidence did not match expected values")
    for item in bad:
        print(item)
    sys.exit(2)

print("PASS: Stage 9W evidence confirms live status stability")
PY
if [ "$?" = "0" ]; then
  pass "Stage 9W evidence values confirmed"
else
  check_fail "Stage 9W evidence validation failed"
fi
...
do
  if grep -Fq "$needle" "$STAGE9X_REPORT"; then
    pass "Stage 9X report contains: $needle"
  else
    check_fail "Stage 9X report missing required text: $needle"
  fi
done
...
health_code="$(curl -sS --max-time 5 -o /tmp/stage9y-health.out -w "%{http_code}" "$BASE/health" 2>/tmp/stage9y-health.err || printf 'curl_failed')"
...
status_code="$(curl -sS --max-time 5 -o "$LIVE_STATUS" -w "%{http_code}" "$ROLLOUT_STATUS_URL" 2>/tmp/stage9y-rollout-status.err || printf 'curl_failed')"
...
status_mutation_code="$(curl -sS --max-time 5 -X POST \
  ...
  -d '{"enabled":true}' \
  -o /tmp/stage9y-rollout-status-mutation.out \
  -w "%{http_code}" \
  "$ROLLOUT_STATUS_URL" 2>/tmp/stage9y-rollout-status-mutation.err || printf 'curl_failed')"
...
request_mutation_code="$(curl -sS --max-time 5 -X POST \
  -H 'Content-Type: application/json' \
  -d '{"enabled":true}' \
  -o /tmp/stage9y-rollout-request-mutation.out \
  -w "%{http_code}" \
  "$BASE/api/router/persistent-rollout/request" 2>/tmp/stage9y-rollout-request-mutation.err || printf 'curl_failed')"
...
post_code="$(curl -sS --max-time 5 -X POST \
  -H 'Content-Type: application/json' \
  -d '{"text":"stage9y mutation boundary plan checkpoint","source":"stage9y","surface":"backend-only"}' \
  -o /tmp/stage9y-router-post.out \
  -w "%{http_code}" \
  "$BASE/api/router/dry-run" 2>/tmp/stage9y-router-post.err || printf 'curl_failed')"
...
live_app_code="$(curl -sS --max-time 8 -o "$LIVE_APP" -w "%{http_code}" "$FRONTEND_BASE/app.js" 2>/tmp/stage9y-live-app.err || printf 'curl_failed')"
live_stub_code="$(curl -sS --max-time 8 -o "$LIVE_STUB" -w "%{http_code}" "$FRONTEND_BASE/router_shadow_read_stub.js" 2>/tmp/stage9y-live-stub.err || printf 'curl_failed')"
...
queue_code="$(curl -sS --max-time 5 -o /tmp/stage9y-system-status.json -w "%{http_code}" "$STATUS_URL" 2>/tmp/stage9y-system-status.err || printf 'curl_failed')"
echo "queue_status_code=$queue_code"

if [ "$queue_code" = "200" ]; then
  python3 - /tmp/stage9y-system-status.json <<'PY'
import json
import sys
...
echo
echo "=== Stage 9Y smoke result ==="
if [ "$fail" = "0" ]; then
  echo "PASS: Stage 9Y disabled mutation-boundary implementation plan verified without implementation"
else
  echo "FAIL: Stage 9Y disabled mutation-boundary implementation plan found issues"
fi
