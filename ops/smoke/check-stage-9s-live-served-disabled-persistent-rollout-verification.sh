...
REPORT="docs/generated/stage-9s-live-served-disabled-persistent-rollout-verification.md"
EVIDENCE="docs/generated/stage-9s-live-served-disabled-persistent-rollout-verification-evidence.json"
SMOKE="ops/smoke/check-stage-9s-live-served-disabled-persistent-rollout-verification.sh"
...
LIVE_APP="/tmp/stage9s-live-app.js"
LIVE_STUB="/tmp/stage9s-live-router-shadow-read-stub.js"
LIVE_BLOCK="/tmp/stage9s-live-persistent-rollout-block.js"

export STAGE9S_FINAL_RESULT="unknown"
export STAGE9S_LIVE_APP_CODE="unknown"
export STAGE9S_LIVE_STUB_CODE="unknown"
export STAGE9S_LIVE_DISABLED_RUNTIME="unknown"
export STAGE9S_HEALTH_CODE="unknown"
export STAGE9S_POST_CODE="unknown"
export STAGE9S_ENV_ABSENT="unknown"
export STAGE9S_QUEUE_CLEAN="unknown"
...
data = {
    ...
    "purpose": "live-served disabled persistent rollout verification",
    "live_app_code": os.environ.get("STAGE9S_LIVE_APP_CODE"),
    "live_stub_code": os.environ.get("STAGE9S_LIVE_STUB_CODE"),
    "live_disabled_runtime": os.environ.get("STAGE9S_LIVE_DISABLED_RUNTIME"),
    "health_code": os.environ.get("STAGE9S_HEALTH_CODE"),
    "post_code": os.environ.get("STAGE9S_POST_CODE"),
    "env_absent": os.environ.get("STAGE9S_ENV_ABSENT"),
    "queue_clean": os.environ.get("STAGE9S_QUEUE_CLEAN"),
    "final_result": os.environ.get("STAGE9S_FINAL_RESULT"),
}
...
echo "=== Stage 9S smoke: live-served disabled persistent rollout verification ==="

echo
echo "=== report/script checks ==="
[ -f "$REPORT" ] && pass "Stage 9S report exists" || check_fail "Stage 9S report missing"
[ -x "$SMOKE" ] && pass "Stage 9S smoke script is executable" || check_fail "Stage 9S smoke script missing or not executable"
...
if bad:
    print("CHECK: Stage 9R evidence did not match expected values")
    for item in bad:
        print(item)
    sys.exit(2)

print("PASS: Stage 9R evidence confirms disabled persistent rollout boundary")
PY
if [ "$?" = "0" ]; then
  pass "Stage 9R evidence values confirmed"
else
  check_fail "Stage 9R evidence validation failed"
fi
...
live_app_code="$(curl -sS --max-time 8 -o "$LIVE_APP" -w "%{http_code}" "$FRONTEND_BASE/app.js" 2>/tmp/stage9s-live-app.err || printf 'curl_failed')"
live_stub_code="$(curl -sS --max-time 8 -o "$LIVE_STUB" -w "%{http_code}" "$FRONTEND_BASE/router_shadow_read_stub.js" 2>/tmp/stage9s-live-stub.err || printf 'curl_failed')"
STAGE9S_LIVE_APP_CODE="$live_app_code"
STAGE9S_LIVE_STUB_CODE="$live_stub_code"
export STAGE9S_LIVE_APP_CODE STAGE9S_LIVE_STUB_CODE
...
if [ "$live_app_code" = "200" ]; then
  ...
  grep -q "PERSISTENT_OPERATOR_GATED_ROLLOUT_ENABLED = false" "$LIVE_APP" 2>/dev/null \
    && pass "live-served persistent rollout default false marker exists" \
    || check_fail "live-served persistent rollout default false marker missing"

  grep -q "persistent_operator_gated_rollout_disabled" "$LIVE_APP" 2>/dev/null \
    && pass "live-served persistent rollout disabled reason exists" \
    || check_fail "live-served persistent rollout disabled reason missing"
...
echo
echo "=== live-served disabled persistent rollout runtime check ==="
if command -v node >/dev/null 2>&1 && [ "$live_app_code" = "200" ]; then
  python3 - "$LIVE_APP" "$LIVE_BLOCK" <<'PY'
from pathlib import Path
import sys
...
print(f"wrote live Stage 9R block: {out}")
PY
  extract_check=$?
...
const block = fs.readFileSync("/tmp/stage9s-live-persistent-rollout-block.js", "utf8");
  ...
  window: {
    ...
    EdgeRouterShadowRead: {
      sendRouterDryRunShadowRead: async () => {
        stubCalls += 1;
        throw new Error("sendRouterDryRunShadowRead should not be called while persistent rollout disabled");
      }
    }
  },
  fetch: async () => {
    fetchCalls += 1;
    throw new Error("fetch should not be called while persistent rollout disabled");
  }
};

vm.createContext(sandbox);
vm.runInContext(block, sandbox, { filename: "stage9s-live-persistent-rollout-block.js" });
...
if (fetchCalls !== 0) throw new Error("fetch was called while persistent rollout disabled");
if (stubCalls !== 0) throw new Error("stub helper was called while persistent rollout disabled");

console.log("PASS: live-served disabled persistent rollout boundary skipped without fetch or stub call");
NODE
    runtime_check=$?
  else
    runtime_check=1
  ...
  if [ "$runtime_check" = "0" ]; then
    STAGE9S_LIVE_DISABLED_RUNTIME="pass"
    export STAGE9S_LIVE_DISABLED_RUNTIME
    pass "live-served disabled persistent rollout runtime check passed"
  else
    STAGE9S_LIVE_DISABLED_RUNTIME="fail"
    export STAGE9S_LIVE_DISABLED_RUNTIME
    check_fail "live-served disabled persistent rollout runtime check failed"
  fi
else
  STAGE9S_LIVE_DISABLED_RUNTIME="not_run"
  export STAGE9S_LIVE_DISABLED_RUNTIME
  check_fail "node or live-served app.js unavailable for runtime check"
fi

echo
echo "=== backend disabled state checks ==="
health_code="$(curl -sS --max-time 5 -o /tmp/stage9s-health.out -w "%{http_code}" "$BASE/health" 2>/tmp/stage9s-health.err || printf 'curl_failed')"
STAGE9S_HEALTH_CODE="$health_code"
export STAGE9S_HEALTH_CODE
...
post_code="$(curl -sS --max-time 5 -X POST \
  ...
  -d '{"text":"stage9s live disabled persistent rollout checkpoint","source":"stage9s","surface":"backend-only"}' \
  -o /tmp/stage9s-router-post.out \
  -w "%{http_code}" \
  "$BASE/api/router/dry-run" 2>/tmp/stage9s-router-post.err || printf 'curl_failed')"
STAGE9S_POST_CODE="$post_code"
export STAGE9S_POST_CODE
...
if printf '%s\n' "$controller_env" | tr ' ' '\n' | grep -qx 'EDGE_UNIVERSAL_INTENT_ROUTER_DRY_RUN_ENABLED=1'; then
  STAGE9S_ENV_ABSENT="false"
  export STAGE9S_ENV_ABSENT
  check_fail "backend dry-run env is enabled"
else
  STAGE9S_ENV_ABSENT="true"
  export STAGE9S_ENV_ABSENT
...
queue_code="$(curl -sS --max-time 5 -o /tmp/stage9s-system-status.json -w "%{http_code}" "$STATUS_URL" 2>/tmp/stage9s-system-status.err || printf 'curl_failed')"
echo "queue_status_code=$queue_code"

if [ "$queue_code" = "200" ]; then
  python3 - /tmp/stage9s-system-status.json <<'PY'
import json
import sys
...
echo
echo "=== write Stage 9S evidence ==="
if [ "$fail" = "0" ]; then
  STAGE9S_FINAL_RESULT="pass"
else
  STAGE9S_FINAL_RESULT="fail"
fi
export STAGE9S_FINAL_RESULT
write_evidence

echo
echo "=== Stage 9S smoke result ==="
if [ "$fail" = "0" ]; then
  echo "PASS: Stage 9S live-served disabled persistent rollout verified"
else
  echo "FAIL: Stage 9S live-served disabled persistent rollout found issues"
fi
