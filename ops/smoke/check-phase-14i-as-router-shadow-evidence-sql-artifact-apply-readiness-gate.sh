...
PHASE="phase-14i-as-router-shadow-evidence-sql-artifact-apply-readiness-gate"
DOC="docs/${PHASE}.md"
SMOKE="ops/smoke/check-${PHASE}.sh"
SQL="ops/db/laptop-app-schema-v3-router-shadow-evidence.sql"

echo "=== Phase 14I-AS router shadow evidence SQL artifact apply-readiness gate ==="
...
echo
echo "=== backup/restore surfaces exist ==="
...
echo "PASS: backup/restore surfaces found"
...
echo
echo "=== apply-readiness documentation markers ==="
python3 - <<'PY'
from pathlib import Path

doc = Path("docs/phase-14i-as-router-shadow-evidence-sql-artifact-apply-readiness-gate.md").read_text()
...
print("PASS: apply-readiness documentation markers verified")
PY
...
echo
echo "=== changed files limited to Phase 14I-AS ==="
python3 - <<'PY'
import subprocess

expected = {
    "docs/phase-14i-as-router-shadow-evidence-sql-artifact-apply-readiness-gate.md",
    "ops/smoke/check-phase-14i-as-router-shadow-evidence-sql-artifact-apply-readiness-gate.sh",
}
...
if unexpected:
    raise SystemExit("FAIL: unexpected changed files for Phase 14I-AS:\n" + "\n".join(unexpected))

print("PASS: changed files are limited to Phase 14I-AS docs/smoke")
PY
...
smoke = Path("ops/smoke/check-phase-14i-as-router-shadow-evidence-sql-artifact-apply-readiness-gate.sh").read_text()
...
echo
echo "=== done: Phase 14I-AS router shadow evidence SQL artifact apply-readiness gate smoke complete ==="
