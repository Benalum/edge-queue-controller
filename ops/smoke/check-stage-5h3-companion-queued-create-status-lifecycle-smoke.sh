...
grep -Fq "STAGE_5H3_WRAPPER_SESSION_COOKIE_USER_LOOKUP_V1" "$WRAPPER" || fail "wrapper missing Stage 5H-3 session cookie lookup marker"
grep -Fq "STAGE_5H3_WRAPPER_LOCAL_SESSION_RESOLVER_FALLBACK_V1" "$WRAPPER" || fail "wrapper missing Stage 5H-3 local session resolver fallback marker"
grep -Fq "STAGE_5H3_WRAPPER_REPO_ROOT_IMPORT_PATH_V1" "$WRAPPER" || fail "wrapper missing Stage 5H-3 repo-root import path marker"
grep -Fq "STAGE_5H3_WRAPPER_CONTROLLER_JSON_TRUSTED_HEADER_INJECTION_V1" "$WRAPPER" || fail "wrapper missing Stage 5H-3 controller-json trusted header injection marker"
grep -Fq "STAGE_5H3_WRAPPER_CONTROLLER_JSON_DIRECT_TRUSTED_HEADERS_V1" "$WRAPPER" || fail "wrapper missing Stage 5H-3 direct controller-json trusted headers marker"
grep -Fq "STAGE_5H3_WRAPPER_NORMALIZE_CT101_EDGE_USER_ID_V1" "$WRAPPER" || fail "wrapper missing Stage 5H-3 CT101 edge user id normalization marker"
...
grep -Fq "STAGE_5H3_TRUSTED_CT101_AUTH_OWNER_PREFERENCE_V1" "$CONTROLLER" || fail "controller missing Stage 5H-3 trusted CT101 auth-owner preference marker"
grep -Fq "STAGE_5H3_CT101_MIRROR_EMAIL_COLLISION_GUARD_V1" "$CONTROLLER" || fail "controller missing Stage 5H-3 CT101 mirror email collision guard marker"
