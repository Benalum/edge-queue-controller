...
{
echo "=== Stage 16 E3D smoke: model inventory/manifests no-apply ==="
...
for spec in "/:200" "/login:200" "/api/me:401" "/api/system/status:200" "/system/status:200"; do
  path="${spec%:*}"
  expect="${spec##*:}"
  code="$(curl -sS -o /tmp/apc_e3d_smoke_public_body.txt -w '%{http_code}' --max-time 20 "${DOMAIN}${path}" 2>/tmp/apc_e3d_smoke_public_err.txt || true)"
  echo "public_path=$path http=${code:-<curl_failed>} expect=$expect"
  [ "$code" = "$expect" ] || mark_fail "public route mismatch: $path"
done
rm -f /tmp/apc_e3d_smoke_public_body.txt /tmp/apc_e3d_smoke_public_err.txt
...
echo
echo "--- PVESO model inventory no-apply ---"
...
curl -fsS --max-time 10 http://127.0.0.1:11434/api/version \
...
curl -fsS --max-time 20 http://127.0.0.1:11434/api/tags \
...
PVESO_REMOTE
  ...
  echo "$pveso_out" | grep -q "api_tags_model_count=2" || mark_fail "expected 2 model tags"
  echo "$pveso_out" | grep -q "qwen2.5-coder:32b-instruct-q4_K_M" || mark_fail "qwen2.5-coder model missing"
  echo "$pveso_out" | grep -q "qwen2.5:32b-instruct-q4_K_M" || mark_fail "qwen2.5 model missing"
