...
echo "=== Phase 14J-I disabled lane filter call plan ==="

PHASE="phase-14j-i-disabled-lane-filter-call-plan"
DOC="docs/${PHASE}.md"
...
echo "PASS: required 14J-I docs/smoke/runtime files exist"
...
doc = Path("docs/phase-14j-i-disabled-lane-filter-call-plan.md").read_text()
...
echo
echo "=== current scheduler gate exists but filter call is still absent ==="
python3 - <<'PY'
from pathlib import Path
import ast
...
if missing:
    raise SystemExit("FAIL: missing current scheduler gate markers: " + ", ".join(missing))
...
if found:
    raise SystemExit("FAIL: scheduler already references filter/metadata helpers: " + ", ".join(found))

print("PASS: scheduler has gate skeleton but no filter/metadata calls")
PY
...
echo
echo "=== helper block isolation remains bounded ==="
python3 - <<'PY'
from pathlib import Path
...
echo
echo "=== changed files limited to Phase 14J-I docs/smoke ==="
python3 - <<'PY'
import subprocess
...
print("PASS: changed files are limited to Phase 14J-I docs/smoke")
PY
...
smoke = Path("ops/smoke/check-phase-14j-i-disabled-lane-filter-call-plan.sh").read_text()
...
if bad:
    raise SystemExit("FAIL: 14J-I smoke contains forbidden live/external behavior:\n" + "\n".join(bad))

print("PASS: smoke behavior guard passed")
PY

echo
echo "=== done: Phase 14J-I disabled lane filter call plan smoke complete ==="
