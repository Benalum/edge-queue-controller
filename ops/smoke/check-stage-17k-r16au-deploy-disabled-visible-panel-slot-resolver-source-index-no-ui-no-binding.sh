...
SLOT_RESOLVER_ASSET="${STATIC_ROOT}/privatepages/study-card-images-disabled-visible-panel-slot-resolver.js"
...
SLOT_RESOLVER_MARKER="APC_STUDY_CARD_IMAGES_DISABLED_VISIBLE_PANEL_SLOT_RESOLVER_R16AS_SOURCE_ONLY"
...
SLOT_RESOLVER_CACHE_BUST="stage17k-r16at-load-disabled-visible-panel-slot-resolver-source-index-source-only-20260708"
...
SLOT_RESOLVER_SCRIPT="/privatepages/study-card-images-disabled-visible-panel-slot-resolver.js?v=${SLOT_RESOLVER_CACHE_BUST}"

echo "=== stage-17k-r16au-deploy-disabled-visible-panel-slot-resolver-source-index-no-ui-no-binding smoke ==="
...
[ -f "$SLOT_RESOLVER_ASSET" ] || { echo "FAIL: slot resolver asset missing"; exit 1; }
...
grep -Fq "$SLOT_RESOLVER_MARKER" "$SLOT_RESOLVER_ASSET" || { echo "FAIL: slot resolver marker missing"; exit 1; }
...
grep -Fq "$SLOT_RESOLVER_SCRIPT" "$INDEX" || { echo "FAIL: slot resolver script missing from index"; exit 1; }
...
slot_resolver_line="$(grep -nF "$SLOT_RESOLVER_SCRIPT" "$INDEX" | head -1 | cut -d: -f1 || true)"
...
[ -n "$slot_resolver_line" ] || { echo "FAIL: slot resolver line missing"; exit 1; }
...
[ "$dom_template_line" -lt "$slot_resolver_line" ] || { echo "FAIL: DOM template must load before slot resolver"; exit 1; }

for f in "$VISIBLE_ASSET" "$ADAPTER_ASSET" "$DOM_TEMPLATE_ASSET" "$SLOT_RESOLVER_ASSET"; do
...
echo "PASS stage-17k-r16au-deploy-disabled-visible-panel-slot-resolver-source-index-no-ui-no-binding smoke"
