...
echo
echo "=== helper count ==="
count="$(grep -c '_stage5p13u_disabled_profile_preferences_ui_support_contract' edge_controller.py || true)"
...
if [ -n "$frontend_diff" ]; then
  echo "$frontend_diff"
  echo "FAIL: disabled Phase 13U should not modify frontend/public/static files"
  fail=1
else
...
if grep -nE '@app\.(get|post|put|patch)\("/api/profile/(preferences|study-preferences|companion-preferences|voice-settings)"|CREATE TABLE.*app_user_preferences|ALTER TABLE.*preferences|INSERT INTO.*app_user_preferences|UPDATE.*app_user_preferences|UPDATE app_users.*preferred_language|UPDATE app_users.*study_language|UPDATE app_users.*learning_style' edge_controller.py; then
  echo "FAIL: live preference route/schema/writes should not exist"
  fail=1
else
  echo "PASS: no live preference routes/schema/writes were added"
fi
...
if [ -n "$frontend_live_markers" ]; then
  ...
  echo "FAIL: disabled Phase 13U should not wire frontend preference API markers"
  fail=1
else
