...
PHASE="phase-14i-ao-router-shadow-evidence-controller-owned-migration-target-inspection"
DOC="docs/${PHASE}.md"
SMOKE="ops/smoke/check-${PHASE}.sh"

echo "=== Phase 14I-AO router shadow evidence controller-owned migration target inspection ==="
...
echo
echo "=== existing database surface files ==="
for f in \
...
echo
echo "=== controller-owned target documentation markers ==="
python3 - <<'PY'
from pathlib import Path

doc = Path("docs/phase-14i-ao-router-shadow-evidence-controller-owned-migration-target-inspection.md").read_text()
...
if found:
    raise SystemExit("FAIL: doc contains concrete SQL instead of target inspection: " + ", ".join(found))

print("PASS: controller-owned target documentation markers verified")
PY
...
expected = {
    "docs/phase-14i-ao-router-shadow-evidence-controller-owned-migration-target-inspection.md",
    "ops/smoke/check-phase-14i-ao-router-shadow-evidence-controller-owned-migration-target-inspection.sh",
}
...
print("PASS: changed files are limited to Phase 14I-AO docs/smoke")
PY
...
smoke = Path("ops/smoke/check-phase-14i-ao-router-shadow-evidence-controller-owned-migration-target-inspection.sh").read_text()
...
echo
echo "=== done: Phase 14I-AO router shadow evidence controller-owned migration target inspection smoke complete ==="
