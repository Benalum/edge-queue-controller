...
echo "=== Phase 14J safe static fast baseline smoke ==="
echo "MUTATION_SCOPE=read_only_static_fast_baseline"
echo "NO CT101 call"
...
smokes=(
  ...
  "ops/smoke/check-phase-14j-bu-no-cleanup-archive-static-inventory-output.sh"
)

for smoke in "${smokes[@]}"; do
  echo
  echo "=== fast baseline: $smoke ==="
  test -x "$smoke"
  bash "$smoke"
done

echo
echo "PASS: Phase 14J safe static fast baseline smoke passed"
