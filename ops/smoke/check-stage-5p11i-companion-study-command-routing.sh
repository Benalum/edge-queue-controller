...
stage5p11i_smoke_main() {
  ...
  echo "=== Stage 5P-11I Companion Study Command Routing Smoke ==="
  ...
  for marker in \
    ...
    "stage5p11iLooksLikeStudyCommand" \
    "stage5p11iRouteCompanionStudyCommand" \
    "/api/study/session/command" \
    "Study command complete" \
    "Study session command" \
    "/api/chat/queued" \
    "queuedChatSubmit"
  do
    if grep -R -Fq "$marker" frontend/wrapper-ui/app.js docs/stage-5p11i-companion-study-command-routing.md; then
  ...
  echo
  echo "=== ensure Companion UI stayed clean ==="
  if grep -Fq 'data-stage5p11i-test' frontend/wrapper-ui/app.js; then
  ...
  if grep -Fq 'button type="button" data-stage5p11i' frontend/wrapper-ui/app.js; then
    echo "FAIL Stage 5P-11I added Companion buttons"
    ok=0
  else
    echo "OK no Stage 5P-11I Companion buttons"
  fi

  echo
  echo "=== verify study hook comes before queued fetch in submit flow ==="
  "$PYBIN" - <<'PY'
from pathlib import Path
s = Path("frontend/wrapper-ui/app.js").read_text()
hook = s.index("STAGE_5P11I_COMPANION_STUDY_COMMAND_ROUTING_SUBMIT_HOOK_BEGIN")
queued = s.index('fetch("/api/chat/queued"', hook)
assert hook < queued
print("OK hook precedes queued fetch")
PY
  if [ "$?" = "0" ]; then
    echo "OK submit hook order"
  else
    echo "FAIL submit hook order"
    ok=0
  fi

  echo
  echo "=== backend command endpoint smoke ==="
  "$PYBIN" - "$base" <<'PY'
import json
import secrets
import sqlite3
...
def create_user_deck_cards(stamp):
    ...
    email = f"stage5p11i-smoke-{stamp}@example.test"
    ...
    with sqlite3.connect(ec.DB_PATH) as conn:
        ...
        if "display_name" in user_columns:
            values["display_name"] = "Stage 5P-11I Smoke User"
        if "password_hash" in user_columns:
            values["password_hash"] = ec._auth_hash_password(f"Stage5P11I-{stamp}-password")
        ...
        conn.execute(
            ...
            (user_id, ec._auth_hash_token(raw_token), now.isoformat(), expires.isoformat(), now.isoformat(), "stage5p11i-smoke"),
        )

        deck_cur = conn.execute(
            "INSERT INTO study_decks (user_id, title, description, created_at, updated_at, archived_at) VALUES (?, ?, ?, ?, ?, NULL)",
            (user_id, f"Stage 5P-11I Deck {stamp}", "companion study routing", now.isoformat(), now.isoformat()),
        )
...
print("OK study commands usable through same endpoint Companion will call")
PY

  if [ "$?" = "0" ]; then
    echo "OK backend command endpoint"
  else
    echo "FAIL backend command endpoint"
    ok=0
  fi

  echo
  echo "=== route smoke ==="
  if curl -fsS "$base/api/system/public-status" >/tmp/stage5p11i-public-status.json; then
  ...
  for route in /companion /study /chat /profile /support /credits /admin /system; do
    code="$(curl -sS -L -o /tmp/stage5p11i-route.html -w "%{http_code}" "$base$route" || true)"
    bytes="$(wc -c < /tmp/stage5p11i-route.html 2>/dev/null || printf 0)"
  ...
  curl -fsS "$base/app.js" >/tmp/stage5p11i-app.js || ok=0

  if grep -Fq "STAGE_5P11I_COMPANION_STUDY_COMMAND_ROUTING_BEGIN" /tmp/stage5p11i-app.js; then
  ...
  echo
  if [ "$ok" = "1" ]; then
    echo "STAGE_5P11I_SMOKE_OK"
    return 0
  fi

  echo "STAGE_5P11I_SMOKE_FAIL"
  return 1
}

if stage5p11i_smoke_main "$@"; then
