...
DOC="docs/phase-14j-nj-pvew-quorum-apply-runbook-no-apply.md"
...
echo "=== Phase 14J-NJ smoke: PVEW quorum apply runbook no-apply ==="
...
test -f "$DOC"
grep -Fq "PVEW Quorum Apply Runbook No-Apply" "$DOC"
grep -Fq "Preferred durable architecture decision" "$DOC"
grep -Fq "Required no-apply inspection before any apply" "$DOC"
grep -Fq "Apply option A" "$DOC"
grep -Fq "Apply option B" "$DOC"
grep -Fq "Rollback requirements" "$DOC"
grep -Fq "Post-apply verification requirements" "$DOC"
...
grep -Fq "no model endpoint called" "$DOC"
echo "PASS: runbook doc required content present"
...
echo "RESULT=PASS_PHASE_14J_NJ_QUORUM_APPLY_RUNBOOK_SMOKE"
