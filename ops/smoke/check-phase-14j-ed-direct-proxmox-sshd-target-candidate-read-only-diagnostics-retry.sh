...
PHASE="phase-14j-ed-direct-proxmox-sshd-target-candidate-read-only-diagnostics-retry"
DOC="docs/${PHASE}.md"
DB="edge_queue.sqlite3"
SERVICE="edge-queue-controller.service"

echo "=== Phase 14J-ED smoke: direct Proxmox sshd target candidate read-only diagnostics retry ==="

test -f "$DOC"
echo "PASS: ED doc exists"
...
for marker in \
  ...
  "NEXT_SAFE_PHASE=direct_proxmox_sshd_candidate_discovery_plan"; do
...
echo "PASS: production runtime remains unchanged after ED checkpoint"
echo "PASS: Phase 14J-ED direct Proxmox sshd target candidate retry smoke passed"
