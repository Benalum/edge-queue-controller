...
DOC="docs/stage-16-e3z-db-r2-add-service-managed-job-type-to-qwen25-profile-current-head-no-worker-start.md"
...
PREV_JOB_TYPE="stage16_e3z_worker_one_shot_activation_proof"
...
python3 - "$PROFILE" "$JOB_TYPE" "$PREV_JOB_TYPE" <<'PY'
from pathlib import Path
import sys

path = Path(sys.argv[1])
job_type = sys.argv[2]
prev_job_type = sys.argv[3]
...
checks = {
    "qwen25_service_job_type_present": job_type in q25,
    "qwen25_previous_job_type_present": prev_job_type in q25,
    "qwen3_service_job_type_absent": job_type not in q3,
...
print("E3Z_DB_R2_PROFILE_SMOKE_OK=1")
PY

echo "E3Z_DB_R2_SMOKE_OK=1"
