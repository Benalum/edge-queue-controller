...
PHASE="phase-14j-dn-controller-power-start-worker-dry-run"
DOC="docs/${PHASE}.md"
DB="edge_queue.sqlite3"
SERVICE="edge-queue-controller.service"

echo "=== Phase 14J-DN smoke: controller power start-worker dry-run ==="

test -f "$DOC"
echo "PASS: DN doc exists"
...
for marker in \
  ...
  "NEXT_SAFE_PHASE=controller_power_start_worker_dry_run_result_checkpoint"; do
...
echo "PASS: production runtime remains unchanged after DN dry-run"
echo "PASS: Phase 14J-DN controller power start-worker dry-run smoke passed"
