...
REPORT="docs/generated/stage-9x-persistent-activation-mutation-design-plan.md"
SMOKE="ops/smoke/check-stage-9x-persistent-activation-mutation-design-plan.sh"
...
LIVE_STATUS="/tmp/stage9x-persistent-rollout-status.json"
LIVE_APP="/tmp/stage9x-live-app.js"
LIVE_STUB="/tmp/stage9x-live-router-shadow-read-stub.js"

echo "=== Stage 9X smoke: persistent activation mutation design plan only ==="

echo
echo "=== report/script checks ==="
[ -f "$REPORT" ] && pass "Stage 9X report exists" || check_fail "Stage 9X report missing"
[ -x "$SMOKE" ] && pass "Stage 9X smoke script is executable" || check_fail "Stage 9X smoke script missing or not executable"
...
for needle in \
  ...
  "Require a later explicit live activation stage before any browser shadow-read request is sent." \
...
echo
echo "=== Stage 9V/9W evidence validation ==="
...
python3 - "$STAGE9V_EVIDENCE" "$STAGE9W_EVIDENCE" <<'PY'
import json
import sys
from pathlib import Path

stage9v = Path(sys.argv[1])
stage9w = Path(sys.argv[2])
...
print("PASS: Stage 9V and Stage 9W evidence values confirmed")
PY
if [ "$?" = "0" ]; then
  pass "Stage 9V/9W evidence values confirmed"
else
  check_fail "Stage 9V/9W evidence validation failed"
fi
...
health_code="$(curl -sS --max-time 5 -o /tmp/stage9x-health.out -w "%{http_code}" "$BASE/health" 2>/tmp/stage9x-health.err || printf 'curl_failed')"
...
status_code="$(curl -sS --max-time 5 -o "$LIVE_STATUS" -w "%{http_code}" "$ROLLOUT_STATUS_URL" 2>/tmp/stage9x-rollout-status.err || printf 'curl_failed')"
...
mutation_code="$(curl -sS --max-time 5 -X POST \
  ...
  -d '{"enabled":true}' \
  -o /tmp/stage9x-rollout-mutation.out \
  -w "%{http_code}" \
  "$ROLLOUT_STATUS_URL" 2>/tmp/stage9x-rollout-mutation.err || printf 'curl_failed')"
...
post_code="$(curl -sS --max-time 5 -X POST \
  -H 'Content-Type: application/json' \
  -d '{"text":"stage9x mutation design checkpoint","source":"stage9x","surface":"backend-only"}' \
  -o /tmp/stage9x-router-post.out \
  -w "%{http_code}" \
  "$BASE/api/router/dry-run" 2>/tmp/stage9x-router-post.err || printf 'curl_failed')"
...
live_app_code="$(curl -sS --max-time 8 -o "$LIVE_APP" -w "%{http_code}" "$FRONTEND_BASE/app.js" 2>/tmp/stage9x-live-app.err || printf 'curl_failed')"
live_stub_code="$(curl -sS --max-time 8 -o "$LIVE_STUB" -w "%{http_code}" "$FRONTEND_BASE/router_shadow_read_stub.js" 2>/tmp/stage9x-live-stub.err || printf 'curl_failed')"
...
queue_code="$(curl -sS --max-time 5 -o /tmp/stage9x-system-status.json -w "%{http_code}" "$STATUS_URL" 2>/tmp/stage9x-system-status.err || printf 'curl_failed')"
echo "queue_status_code=$queue_code"

if [ "$queue_code" = "200" ]; then
  python3 - /tmp/stage9x-system-status.json <<'PY'
import json
import sys
...
echo
echo "=== Stage 9X smoke result ==="
if [ "$fail" = "0" ]; then
  echo "PASS: Stage 9X persistent activation mutation design plan verified without implementation"
else
  echo "FAIL: Stage 9X persistent activation mutation design plan found issues"
fi
