...
echo "=== CT101 dormant client one-shot smoke ==="
...
PORT="${LAPTOP_QUEUE_CT101_CLIENT_ONESHOT_SMOKE_PORT:-7105}"
IDS_FILE="/tmp/ct101-laptop-queue-s5e12-ids-$PORT.json"
...
if command -v tailscale >/dev/null 2>&1; then
  LAPTOP_HOST="${LAPTOP_QUEUE_CLIENT_ONESHOT_HOST:-$(tailscale ip -4 | head -1)}"
else
  LAPTOP_HOST="${LAPTOP_QUEUE_CLIENT_ONESHOT_HOST:-$(hostname -I | awk '{print $1}')}"
fi
...
LOG_FILE="/tmp/ct101-dormant-client-oneshot-smoke-$PORT.log"
...
if parsed.get("ok") is True and parsed.get("leftover_count") == 0:
    print("OK: cleanup removed synthetic Stage 5E-12 rows")
PY
...
bash ops/smoke/check-laptop-queue-client-one-shot.sh

echo "OK: CT101 dormant client one-shot script is present and valid"
REMOTE
...
suffix = f"s5e12-{int(time.time())}-{os.getpid()}"

payload = {
    "suffix": suffix,
    "job_type": "ollama_chat",
    "requested_model": "stage-5e12-synthetic-model",
}
...
print("OK: laptop API created synthetic jobs for dormant client")
PY

IDS_B64="$(base64 -w0 "$IDS_FILE")"

echo "Running CT101 dormant client success path"
...
LAPTOP_QUEUE_CLIENT_ONE_SHOT_RESULT="success" \
python3 ops/smoke/laptop_queue_client_one_shot.py
REMOTE

echo "Running CT101 dormant client failure path"
...
LAPTOP_QUEUE_WORKER_ID="$WORKER_ID" \
LAPTOP_QUEUE_JOB_TYPE="ollama_chat" \
LAPTOP_QUEUE_CLIENT_ONE_SHOT_RESULT="failure" \
python3 ops/smoke/laptop_queue_client_one_shot.py
REMOTE
...
echo "PASS: CT101 dormant client one-shot smoke passed"
