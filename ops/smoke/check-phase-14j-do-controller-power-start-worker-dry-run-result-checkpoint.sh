...
PHASE="phase-14j-do-controller-power-start-worker-dry-run-result-checkpoint"
DOC="docs/${PHASE}.md"
DB="edge_queue.sqlite3"
SERVICE="edge-queue-controller.service"

echo "=== Phase 14J-DO smoke: controller power start-worker dry-run result checkpoint ==="

test -f "$DOC"
echo "PASS: DO doc exists"
...
for marker in \
  ...
  "NEXT_SAFE_PHASE=guarded_worker_start_decision_plan"; do
...
echo "PASS: production runtime remains unchanged after DO"
echo "PASS: Phase 14J-DO controller power start-worker dry-run result checkpoint smoke passed"
