...
echo "=== Phase 11F smoke: System loading completion repair ==="

fail=0

code_for() {
  local url="$1"
  curl -sS --max-time 5 -o /tmp/phase11f-curl.out -w "%{http_code}" "$url" 2>/tmp/phase11f-curl.err || true
}

echo
echo "=== verify stage files exist ==="
for path in \
  docs/phase-11f-system-loading-completion-repair.md \
  ops/smoke/check-phase-11f-system-loading-completion-repair.sh \
...
echo
echo "=== verify Phase 11F markers and repair wiring ==="
marker_count="$(grep -c 'PHASE_11F_SYSTEM_LOADING_COMPLETION_REPAIR_BEGIN' frontend/wrapper-ui/app.js || true)"
if [ "$marker_count" = "1" ]; then
  echo "PASS: Phase 11F marker exists exactly once"
else
  echo "FAIL: Phase 11F marker count is $marker_count"
  fail=1
fi

if grep -q 'id="phase11fSystemStatusLoading"' frontend/wrapper-ui/app.js; then
  echo "PASS: stable loading DOM id exists"
else
  echo "FAIL: stable loading DOM id missing"
  fail=1
fi

if grep -q 'function phase11fRefreshSystemPageIfReady()' frontend/wrapper-ui/app.js; then
  echo "PASS: Phase 11F refresh helper exists"
else
  echo "FAIL: Phase 11F refresh helper missing"
  fail=1
fi

if grep -q 'phase11fRefreshSystemPageIfReady();' frontend/wrapper-ui/app.js; then
  echo "PASS: Phase 11F refresh helper is called"
else
  echo "FAIL: Phase 11F refresh helper call missing"
  fail=1
fi

if grep -q 'function phase11eRenderSystemPageOriginal()' frontend/wrapper-ui/app.js; then
  echo "PASS: Phase 11E preserved System renderer still exists"
else
  echo "FAIL: Phase 11E preserved System renderer missing"
  fail=1
fi
...
unexpected_files="$(git status --short | awk '{print $2}' | grep -vE '^(docs/phase-11f-system-loading-completion-repair\.md|ops/smoke/check-phase-11f-system-loading-completion-repair\.sh|frontend/wrapper-ui/app\.js)$' || true)"
if [ -n "$unexpected_files" ]; then
  echo "FAIL: unexpected changed files:"
  printf '%s\n' "$unexpected_files"
  fail=1
else
  echo "PASS: only expected Phase 11F files changed"
fi
...
if [ -n "$STATUS_BASE" ]; then
  PHASE11F_STATUS_BASE="$STATUS_BASE" python3 - <<'PY'
import json
import os
import statistics
import sys
...
base = os.environ["PHASE11F_STATUS_BASE"].rstrip("/")
...
if mean > 0.500:
    print("FAIL: /api/system/status mean latency exceeded 0.500s Phase 11F threshold")
    sys.exit(1)
...
echo
if [ "$fail" = "0" ]; then
  echo "PASS: Phase 11F smoke passed"
else
  echo "FAIL: Phase 11F smoke failed"
fi
