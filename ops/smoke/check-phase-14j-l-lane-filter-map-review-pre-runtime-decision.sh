...
echo "=== Phase 14J-L lane filter map review pre-runtime decision ==="

PHASE="phase-14j-l-lane-filter-map-review-pre-runtime-decision"
DOC="docs/${PHASE}.md"
SMOKE="ops/smoke/check-${PHASE}.sh"
REVIEW="docs/${PHASE}-candidate-map-review.txt"
MAP="docs/phase-14j-k-lane-filter-candidate-variable-map-select-best-worker-variable-map.txt"
SNAPSHOT="docs/phase-14j-j-lane-filter-exact-insertion-inspection-select-best-worker-snapshot.txt"

echo
echo "=== required files ==="
for f in "$DOC" "$SMOKE" "$REVIEW" "$MAP" "$SNAPSHOT" "edge_controller.py"; do
  test -f "$f"
done
echo "PASS: required 14J-L docs/smoke/review/map/runtime files exist"
...
doc = Path("docs/phase-14j-l-lane-filter-map-review-pre-runtime-decision.md").read_text()

required = [
    "docs/smoke-only review phase",
...
required = [
    ...
    "for_loops:",
    "calls:",
    "pre_runtime_decision:",
    "safe_next_phase_candidate:",
]

missing = [m for m in required if m not in review]
if missing:
    raise SystemExit("FAIL: review missing markers: " + ", ".join(missing))
...
found_sensitive = [x for x in sensitive if x.lower() in review.lower()]
if found_sensitive:
    raise SystemExit("FAIL: review contains sensitive marker: " + ", ".join(found_sensitive))

print("PASS: review artifact is bounded and safe")
PY
...
echo
echo "=== changed files limited to Phase 14J-L docs/smoke/review ==="
python3 - <<'PY'
import subprocess
...
print("PASS: changed files are limited to Phase 14J-L docs/smoke/review")
PY
...
smoke = Path("ops/smoke/check-phase-14j-l-lane-filter-map-review-pre-runtime-decision.sh").read_text()
...
if bad:
    raise SystemExit("FAIL: 14J-L smoke contains forbidden live/external behavior:\n" + "\n".join(bad))

print("PASS: smoke behavior guard passed")
PY

echo
echo "=== done: Phase 14J-L lane filter map review smoke complete ==="
