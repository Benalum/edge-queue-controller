...
echo "=== Phase 14J-BE smoke: synthetic fallback and rollback smoke result checkpoint ==="

DOC="docs/phase-14j-be-synthetic-fallback-and-rollback-smoke-result-checkpoint.md"
...
python3 - <<'APC_BE_DOC_CHECK'
from pathlib import Path

text = Path("docs/phase-14j-be-synthetic-fallback-and-rollback-smoke-result-checkpoint.md").read_text()
...
required = [
    ...
    "Smoke result checkpointing is not runtime activation.",
    "In-process helper testing is not service activation.",
    "A passing synthetic smoke is not approval to enable lane workers.",
...
echo
echo "=== source synthetic smoke result markers ==="
python3 - <<'APC_BE_SOURCE'
from pathlib import Path
...
print("PASS: synthetic smoke source markers remain present and worker payload lane metadata remains blocked")
APC_BE_SOURCE

echo
echo "=== source remains docs/smoke only for BE ==="
if git diff --name-only | grep -q '^edge_controller.py$'; then
  echo "FAIL: edge_controller.py changed during BE"
  fail=1
else
  echo "PASS: edge_controller.py unchanged in BE"
fi

echo
echo "=== run BD smoke artifact as result evidence ==="
...
health_code="$(
  curl -sS --max-time 5 -o /tmp/phase14j-be-local-health.json \
    -w "%{http_code}" \
    http://127.0.0.1:7070/system/local-health 2>/tmp/phase14j-be-local-health.err || true
)"
...
echo
echo "=== static guard: BE smoke must not execute forbidden runtime actions ==="
...
if [ -n "$forbidden_runtime_lines" ]; then
  echo "FAIL: BE smoke contains executable forbidden runtime lines"
  echo "$forbidden_runtime_lines"
  fail=1
else
  echo "PASS: BE smoke contains no executable forbidden runtime lines"
fi

echo
if [ "$fail" = "0" ]; then
  echo "PASS: Phase 14J-BE smoke complete"
else
  echo "FAIL: Phase 14J-BE smoke failed"
fi
