...
echo "=== Phase 14J-BB smoke: no-lane fallback and rollback evidence checkpoint ==="

DOC="docs/phase-14j-bb-no-lane-fallback-and-rollback-evidence-checkpoint.md"
...
python3 - <<'APC_BB_DOC_CHECK'
from pathlib import Path

text = Path("docs/phase-14j-bb-no-lane-fallback-and-rollback-evidence-checkpoint.md").read_text()
...
required = [
    ...
    "Evidence checkpointing is not runtime activation.",
    "Rollback draft evidence is not rollback execution.",
...
echo
echo "=== source fallback markers ==="
python3 - <<'APC_BB_SOURCE'
from pathlib import Path
...
missing = [item for item in required if item not in text]
if missing:
    raise SystemExit("FAIL: missing fallback source markers: " + ", ".join(missing))
...
print("PASS: fallback source markers remain present and worker payload lane metadata remains blocked")
APC_BB_SOURCE

echo
echo "=== source remains docs/smoke only for BB ==="
if git diff --name-only | grep -q '^edge_controller.py$'; then
  echo "FAIL: edge_controller.py changed during BB"
  fail=1
else
  echo "PASS: edge_controller.py unchanged in BB"
fi
...
echo
echo "=== controller job no-lane evidence remains inspectable ==="
if sqlite3 "file:$PWD/$DB?mode=ro" "SELECT name FROM sqlite_master WHERE type='table' AND name='jobs';" | grep -q '^jobs$'; then
  jobs_total="$(sqlite3 "file:$PWD/$DB?mode=ro" "SELECT COUNT(*) FROM jobs;")"
  echo "jobs_total=$jobs_total"
  if [ "$jobs_total" -ge "1" ]; then
    echo "PASS: jobs table remains inspectable"
  else
    echo "CHECK: jobs table exists but currently has no rows"
  fi
else
  echo "CHECK: no jobs table exists"
fi
...
health_code="$(
  curl -sS --max-time 5 -o /tmp/phase14j-bb-local-health.json \
    -w "%{http_code}" \
    http://127.0.0.1:7070/system/local-health 2>/tmp/phase14j-bb-local-health.err || true
)"
...
echo
echo "=== static guard: BB smoke must not execute forbidden runtime actions ==="
...
if [ -n "$forbidden_runtime_lines" ]; then
  echo "FAIL: BB smoke contains executable forbidden runtime lines"
  echo "$forbidden_runtime_lines"
  fail=1
else
  echo "PASS: BB smoke contains no executable forbidden runtime lines"
fi

echo
if [ "$fail" = "0" ]; then
  echo "PASS: Phase 14J-BB smoke complete"
else
  echo "FAIL: Phase 14J-BB smoke failed"
fi
