...
DOC="docs/stage-17k-r15a-deploy-disabled-save-button-mount-plan-asset-not-loaded.md"
OUT_DIR="docs/smoke/generated/stage-17k-r15a-deploy-disabled-save-button-mount-plan-asset-not-loaded"
...
grep -Fq "Deploy Disabled Save Button Mount Plan Asset Not Loaded" "$DOC"
grep -Fq "No index load" "$DOC"
grep -Fq "No live UI change" "$DOC"
grep -Fq "No Profile mount change" "$DOC"
grep -Fq "No panel change" "$DOC"
...
if grep -Fq "/privatepages/local-backup-current-file-disabled-save-button-mount-plan.js" "$INDEX"; then
  echo "FAIL: index must not load disabled save button mount plan"
  exit 1
fi

if grep -Fq "APC_LOCAL_BACKUP_CURRENT_FILE_DISABLED_SAVE_BUTTON_MOUNT_PLAN" "$MOUNT" "$PANEL"; then
  echo "FAIL: mount/panel must not reference disabled save button mount plan"
  exit 1
fi

grep -Fq "R15A_VM200_DISABLED_SAVE_BUTTON_MOUNT_PLAN_ASSET_NOT_LOADED_DEPLOY_DONE" "$OUT_DIR/vm200-deploy."*.txt
grep -Fq "PASS public static R15A smoke" "$OUT_DIR/public-static-smoke."*.txt
...
echo "PASS stage-17k-r15a deploy disabled save button mount plan asset not loaded smoke"
