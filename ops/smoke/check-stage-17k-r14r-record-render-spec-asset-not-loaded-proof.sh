...
DOC="docs/stage-17k-r14r-record-render-spec-asset-not-loaded-proof.md"
OUT_DIR="docs/smoke/generated/stage-17k-r14r-record-render-spec-asset-not-loaded-proof"

test -f "$DOC"
test -d "$OUT_DIR"

grep -Fq "Record Render Spec Asset Not Loaded Proof" "$DOC"
grep -Fq "Browser proof passed" "$DOC"
grep -Fq "PASS_R14Q_RENDER_SPEC_ASSET_NOT_LOADED_NO_UI_NO_BINDING" "$DOC"

grep -Fq "renderSpecLoadedByScript false" "$DOC"
grep -Fq "renderSpecWindowPresent false" "$DOC"
...
grep -Fq "assetHasRenderSpecFunction true" "$DOC"
grep -Fq "assetHasRenderSpecTextFunction true" "$DOC"
...
echo "PASS stage-17k-r14r record render spec asset-not-loaded proof smoke"
