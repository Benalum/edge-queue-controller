...
echo "=== Phase 14J safe static ultra-concise v2 baseline smoke ==="
echo "MUTATION_SCOPE=read_only_static_ultra_concise_v2_baseline"
echo "NO CT101 call"
...
smokes=(
  ...
  "ops/smoke/check-phase-14j-bx-controller-owned-active-source-ui-map.sh"
  "ops/smoke/check-phase-14j-bx-static-ui-copy-layout-readiness-contract.sh"
  "ops/smoke/check-phase-14j-bx-no-cache-active-source-inventory-output.sh"
)

for smoke in "${smokes[@]}"; do
  echo
  echo "=== ultra v2 baseline: $smoke ==="
  test -x "$smoke"
  bash "$smoke"
done

echo
echo "PASS: Phase 14J safe static ultra-concise v2 baseline smoke passed"
