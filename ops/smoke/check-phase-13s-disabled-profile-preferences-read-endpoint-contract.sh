...
echo
echo "=== helper count ==="
count="$(grep -c '_stage5p13s_disabled_profile_preferences_read_endpoint_contract' edge_controller.py || true)"
...
if grep -nE '@app\.(get|post|put|patch)\("/api/profile/(preferences|study-preferences|companion-preferences|voice-settings)"|CREATE TABLE.*app_user_preferences|ALTER TABLE.*preferences|UPDATE app_users.*preferred_language|UPDATE app_users.*study_language|UPDATE app_users.*learning_style' edge_controller.py; then
  echo "FAIL: live preference read route/schema/writes should not exist"
  fail=1
else
  echo "PASS: no live preference read routes/schema/writes were added"
fi
