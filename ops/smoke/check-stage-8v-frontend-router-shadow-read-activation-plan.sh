...
REPORT="docs/generated/stage-8v-frontend-router-shadow-read-activation-plan.md"
SMOKE="ops/smoke/check-stage-8v-frontend-router-shadow-read-activation-plan.sh"
STAGE8T_EVIDENCE="docs/generated/stage-8t-live-backend-router-dry-run-controlled-activation-rollback-evidence.json"
STAGE8U_REPORT="docs/generated/stage-8u-router-dry-run-evidence-review-no-browser-traffic-checkpoint.md"
...
echo "=== Stage 8V smoke: frontend/browser shadow-read activation plan only ==="

echo
echo "=== report/script checks ==="
[ -f "$REPORT" ] && pass "Stage 8V report exists" || check_fail "Stage 8V report missing"
[ -x "$SMOKE" ] && pass "Stage 8V smoke script is executable" || check_fail "Stage 8V smoke script missing or not executable"
...
echo
echo "=== Stage 8T/8U evidence checks ==="
if [ -f "$STAGE8T_EVIDENCE" ]; then
  pass "Stage 8T evidence exists"
else
  check_fail "missing Stage 8T evidence"
fi

if [ -f "$STAGE8U_REPORT" ]; then
  pass "Stage 8U report exists"
else
  check_fail "missing Stage 8U report"
fi
...
if not path.exists():
    print("CHECK: Stage 8T evidence missing")
    sys.exit(1)
...
if bad:
    print("CHECK: Stage 8T evidence values did not match expected")
    for key, expected_value, actual in bad:
        print(f"  {key}: expected={expected_value} actual={actual}")
    sys.exit(2)

print("PASS: Stage 8T evidence still proves backend activation/rollback")
PY
evidence_check=$?
if [ "$evidence_check" = "0" ]; then
  pass "Stage 8T evidence values confirmed"
else
...
health_code="$(curl -sS --max-time 5 -o /tmp/stage8v-health.out -w "%{http_code}" "$BASE/health" 2>/tmp/stage8v-health.err || printf 'curl_failed')"
echo "health_code=$health_code"
if [ "$health_code" = "200" ]; then
  pass "live controller /health returned HTTP 200"
else
  check_fail "live controller /health did not return HTTP 200"
  sed -n '1,80p' /tmp/stage8v-health.err 2>/dev/null || true
  sed -n '1,80p' /tmp/stage8v-health.out 2>/dev/null || true
fi

post_code="$(curl -sS --max-time 5 -X POST \
  -H 'Content-Type: application/json' \
  -d '{"text":"stage8v disabled checkpoint","source":"stage8v","surface":"backend-only"}' \
  -o /tmp/stage8v-router-post.out \
  -w "%{http_code}" \
  "$BASE/api/router/dry-run" 2>/tmp/stage8v-router-post.err || printf 'curl_failed')"
...
if [ "$post_code" = "404" ]; then
  ...
  sed -n '1,120p' /tmp/stage8v-router-post.out 2>/dev/null || true
fi
...
queue_code="$(curl -sS --max-time 5 -o /tmp/stage8v-system-status.json -w "%{http_code}" "$STATUS_URL" 2>/tmp/stage8v-system-status.err || printf 'curl_failed')"
echo "queue_status_code=$queue_code"

if [ "$queue_code" = "200" ]; then
  python3 - /tmp/stage8v-system-status.json <<'PY'
import json
import sys
...
PY
  ...
  if [ "$queue_check" = "0" ]; then
    ...
    sed -n '1,160p' /tmp/stage8v-system-status.json 2>/dev/null || true
  fi
else
  check_fail "could not read queue/system status"
  sed -n '1,80p' /tmp/stage8v-system-status.err 2>/dev/null || true
fi
...
echo
echo "=== Stage 8V smoke result ==="
if [ "$fail" = "0" ]; then
  echo "PASS: Stage 8V frontend/browser shadow-read activation plan verified without enablement"
else
  echo "FAIL: Stage 8V frontend/browser shadow-read activation plan found issues"
fi
