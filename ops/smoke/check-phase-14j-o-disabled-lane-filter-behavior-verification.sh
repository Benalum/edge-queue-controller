...
echo "=== Phase 14J-O disabled lane filter behavior verification ==="

PHASE="phase-14j-o-disabled-lane-filter-behavior-verification"
DOC="docs/${PHASE}.md"
...
echo "PASS: required 14J-O docs/smoke/runtime files exist"
...
doc = Path("docs/phase-14j-o-disabled-lane-filter-behavior-verification.md").read_text()
...
echo
echo "=== isolated disabled gate helper behavior ==="
python3 - <<'PY'
from pathlib import Path
import os
...
workers = [
    ...
    {"worker_id": "study-lane", "worker_role": "lane", "worker_lane": "study", "capabilities": ["ollama_chat"], "state": "available", "accepts_lane_jobs": True},
]
job = {
    "job_lane": "study",
...
for value in disabled_values:
    ...
    if result != workers:
        raise SystemExit(f"FAIL: disabled gate did not pass through workers for value={value!r}")

print("PASS: disabled gate values preserve pass-through behavior")
PY
...
echo
echo "=== changed files limited to Phase 14J-O docs/smoke ==="
python3 - <<'PY'
import subprocess
...
print("PASS: changed files are limited to Phase 14J-O docs/smoke")
PY
...
smoke = Path("ops/smoke/check-phase-14j-o-disabled-lane-filter-behavior-verification.sh").read_text()
...
if bad:
    raise SystemExit("FAIL: 14J-O smoke contains forbidden live/external behavior:\n" + "\n".join(bad))

print("PASS: smoke behavior guard passed")
PY

echo
echo "=== done: Phase 14J-O disabled lane filter behavior verification smoke complete ==="
