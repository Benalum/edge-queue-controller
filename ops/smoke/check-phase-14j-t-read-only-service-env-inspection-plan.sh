...
echo "=== Phase 14J-T read-only service environment inspection plan ==="

PHASE="phase-14j-t-read-only-service-env-inspection-plan"
DOC="docs/${PHASE}.md"
...
echo "PASS: required 14J-T docs/smoke/plan/runtime files exist"
...
doc = Path("docs/phase-14j-t-read-only-service-env-inspection-plan.md").read_text()
plan = Path("docs/phase-14j-t-read-only-service-env-inspection-plan-plan.txt").read_text()
...
echo
echo "=== changed files limited to Phase 14J-T docs/smoke/plan ==="
python3 - <<'PY'
import subprocess

allowed = {
    "docs/phase-14j-t-read-only-service-env-inspection-plan.md",
    "docs/phase-14j-t-read-only-service-env-inspection-plan-plan.txt",
    "ops/smoke/check-phase-14j-t-read-only-service-env-inspection-plan.sh",
}
...
print("PASS: changed files are limited to Phase 14J-T docs/smoke/plan")
PY
...
smoke = Path("ops/smoke/check-phase-14j-t-read-only-service-env-inspection-plan.sh").read_text()
...
if bad:
    raise SystemExit("FAIL: 14J-T smoke contains forbidden live/external behavior:\n" + "\n".join(bad))

print("PASS: smoke behavior guard passed")
PY

echo
echo "=== done: Phase 14J-T read-only service environment inspection plan smoke complete ==="
