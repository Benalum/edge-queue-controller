...
PHASE="phase-14j-ef-direct-proxmox-sshd-candidate-read-only-diagnostics-second-retry"
DOC="docs/${PHASE}.md"
DB="edge_queue.sqlite3"
SERVICE="edge-queue-controller.service"

echo "=== Phase 14J-EF smoke: direct Proxmox sshd candidate read-only diagnostics second retry ==="

test -f "$DOC"
echo "PASS: EF doc exists"
...
for marker in \
  ...
  "NEXT_SAFE_PHASE=proxmox_lan_sshd_reachability_plan"; do
...
echo "PASS: production runtime remains unchanged after EF checkpoint"
echo "PASS: Phase 14J-EF direct Proxmox sshd candidate second retry smoke passed"
