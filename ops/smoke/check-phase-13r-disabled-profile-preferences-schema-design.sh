...
if grep -nE '@app\.(post|get|put|patch)\("/api/profile/(preferences|study-preferences|companion-preferences)"|CREATE TABLE.*preferences|ALTER TABLE.*preferences|UPDATE app_users.*preferred_language|UPDATE app_users.*study_language|UPDATE app_users.*learning_style' edge_controller.py; then
  echo "FAIL: live preference schema/routes/writes should not exist"
  fail=1
else
  echo "PASS: no live preference schema/routes/writes were added"
fi
