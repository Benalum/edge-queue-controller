...
echo "=== Phase 14I-AX router shadow evidence writer helper design ==="

PHASE="phase-14i-ax-router-shadow-evidence-writer-helper-design"
DOC="docs/${PHASE}.md"
...
echo "PASS: required AX docs/smoke and existing SQL/apply artifacts exist"
...
doc = Path("docs/phase-14i-ax-router-shadow-evidence-writer-helper-design.md").read_text()

required = [
    "docs/smoke-only design gate",
...
echo
echo "=== SQL/apply artifacts still present and unchanged in role ==="
python3 - <<'PY'
from pathlib import Path
...
print("PASS: runtime writer markers are absent from edge_controller.py")
PY

echo
echo "=== existing router shadow boundary still static-only ==="
python3 - <<'PY'
from pathlib import Path
...
print("PASS: existing router shadow static markers remain present")
PY

echo
echo "=== changed files limited to Phase 14I-AX docs/smoke ==="
python3 - <<'PY'
import subprocess

allowed = {
    "docs/phase-14i-ax-router-shadow-evidence-writer-helper-design.md",
    "ops/smoke/check-phase-14i-ax-router-shadow-evidence-writer-helper-design.sh",
}
...
print("PASS: changed files are limited to Phase 14I-AX docs/smoke")
PY
...
smoke = Path("ops/smoke/check-phase-14i-ax-router-shadow-evidence-writer-helper-design.sh").read_text()
...
if bad:
    raise SystemExit("FAIL: AX smoke contains forbidden live/apply behavior:\n" + "\n".join(bad))

print("PASS: smoke behavior guard passed")
PY

echo
echo "=== done: Phase 14I-AX router shadow evidence writer helper design smoke complete ==="
