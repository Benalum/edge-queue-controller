...
REPORT="docs/generated/stage-9l-live-served-disabled-operator-gate-verification.md"
EVIDENCE="docs/generated/stage-9l-live-served-disabled-operator-gate-verification-evidence.json"
SMOKE="ops/smoke/check-stage-9l-live-served-disabled-operator-gate-verification.sh"
...
LIVE_APP="/tmp/stage9l-live-app.js"
LIVE_STUB="/tmp/stage9l-live-router-shadow-read-stub.js"
LIVE_BLOCKS="/tmp/stage9l-live-app-blocks.js"

export STAGE9L_FINAL_RESULT="unknown"
export STAGE9L_LIVE_APP_CODE="unknown"
export STAGE9L_LIVE_STUB_CODE="unknown"
export STAGE9L_LIVE_DISABLED_RUNTIME="unknown"
export STAGE9L_HEALTH_CODE="unknown"
export STAGE9L_POST_CODE="unknown"
export STAGE9L_ENV_ABSENT="unknown"
export STAGE9L_QUEUE_CLEAN="unknown"
...
data = {
    ...
    "purpose": "live-served disabled operator gate verification",
    "live_app_code": os.environ.get("STAGE9L_LIVE_APP_CODE"),
    "live_stub_code": os.environ.get("STAGE9L_LIVE_STUB_CODE"),
    "live_disabled_runtime": os.environ.get("STAGE9L_LIVE_DISABLED_RUNTIME"),
    "health_code": os.environ.get("STAGE9L_HEALTH_CODE"),
    "post_code": os.environ.get("STAGE9L_POST_CODE"),
    "env_absent": os.environ.get("STAGE9L_ENV_ABSENT"),
    "queue_clean": os.environ.get("STAGE9L_QUEUE_CLEAN"),
    "final_result": os.environ.get("STAGE9L_FINAL_RESULT"),
}
...
echo "=== Stage 9L smoke: live-served disabled operator gate verification ==="

echo
echo "=== report/script checks ==="
[ -f "$REPORT" ] && pass "Stage 9L report exists" || check_fail "Stage 9L report missing"
[ -x "$SMOKE" ] && pass "Stage 9L smoke script is executable" || check_fail "Stage 9L smoke script missing or not executable"
...
if bad:
    print("CHECK: Stage 9K evidence did not match expected values")
    for item in bad:
        print(item)
    sys.exit(2)

print("PASS: Stage 9K evidence confirms disabled operator gate")
PY
if [ "$?" = "0" ]; then
  pass "Stage 9K evidence values confirmed"
else
  check_fail "Stage 9K evidence validation failed"
fi
...
live_app_code="$(curl -sS --max-time 8 -o "$LIVE_APP" -w "%{http_code}" "$FRONTEND_BASE/app.js" 2>/tmp/stage9l-live-app.err || printf 'curl_failed')"
live_stub_code="$(curl -sS --max-time 8 -o "$LIVE_STUB" -w "%{http_code}" "$FRONTEND_BASE/router_shadow_read_stub.js" 2>/tmp/stage9l-live-stub.err || printf 'curl_failed')"
STAGE9L_LIVE_APP_CODE="$live_app_code"
STAGE9L_LIVE_STUB_CODE="$live_stub_code"
export STAGE9L_LIVE_APP_CODE STAGE9L_LIVE_STUB_CODE
...
echo
echo "=== live-served disabled operator gate runtime check ==="
...
const stub = fs.readFileSync("/tmp/stage9l-live-router-shadow-read-stub.js", "utf8");
const appBlocks = fs.readFileSync("/tmp/stage9l-live-app-blocks.js", "utf8");
...
const sandbox = {
  ...
  navigator: { userAgent: "stage9l-live-disabled-operator-gate" },
  location: { href: "http://127.0.0.1:8787/stage9l" },
  localStorage: {
    getItem: () => null,
    setItem: () => {},
    removeItem: () => {}
...
vm.runInContext(stub, sandbox, { filename: "stage9l-live-stub.js" });
vm.runInContext(appBlocks, sandbox, { filename: "stage9l-live-app-blocks.js" });
...
shadow.sendRouterDryRunShadowRead = async () => {
  stubCalls += 1;
  throw new Error("sendRouterDryRunShadowRead should not be called while operator gate disabled");
};

Promise.resolve(
  surface.requestBrowserSurfaceRouterShadowRead("manual-diagnostic", {
    text: "stage9l live disabled operator gate runtime"
  })
).then((result) => {
  ...
  console.log("PASS: live-served disabled operator gate skipped without fetch or stub call");
}).catch((error) => {
...
NODE
  ...
  if [ "$runtime_check" = "0" ]; then
    STAGE9L_LIVE_DISABLED_RUNTIME="pass"
    export STAGE9L_LIVE_DISABLED_RUNTIME
    pass "live-served disabled operator gate runtime check passed"
  else
    STAGE9L_LIVE_DISABLED_RUNTIME="fail"
    export STAGE9L_LIVE_DISABLED_RUNTIME
    check_fail "live-served disabled operator gate runtime check failed"
  fi
else
  STAGE9L_LIVE_DISABLED_RUNTIME="not_run"
  export STAGE9L_LIVE_DISABLED_RUNTIME
...
health_code="$(curl -sS --max-time 5 -o /tmp/stage9l-health.out -w "%{http_code}" "$BASE/health" 2>/tmp/stage9l-health.err || printf 'curl_failed')"
STAGE9L_HEALTH_CODE="$health_code"
export STAGE9L_HEALTH_CODE
...
post_code="$(curl -sS --max-time 5 -X POST \
  ...
  -d '{"text":"stage9l live disabled operator gate checkpoint","source":"stage9l","surface":"backend-only"}' \
  -o /tmp/stage9l-router-post.out \
  -w "%{http_code}" \
  "$BASE/api/router/dry-run" 2>/tmp/stage9l-router-post.err || printf 'curl_failed')"
STAGE9L_POST_CODE="$post_code"
export STAGE9L_POST_CODE
...
if printf '%s\n' "$controller_env" | tr ' ' '\n' | grep -qx 'EDGE_UNIVERSAL_INTENT_ROUTER_DRY_RUN_ENABLED=1'; then
  STAGE9L_ENV_ABSENT="false"
  export STAGE9L_ENV_ABSENT
  check_fail "backend dry-run env is enabled"
else
  STAGE9L_ENV_ABSENT="true"
  export STAGE9L_ENV_ABSENT
...
queue_code="$(curl -sS --max-time 5 -o /tmp/stage9l-system-status.json -w "%{http_code}" "$STATUS_URL" 2>/tmp/stage9l-system-status.err || printf 'curl_failed')"
echo "queue_status_code=$queue_code"

if [ "$queue_code" = "200" ]; then
  python3 - /tmp/stage9l-system-status.json <<'PY'
import json
import sys
...
echo
echo "=== write Stage 9L evidence ==="
if [ "$fail" = "0" ]; then
  STAGE9L_FINAL_RESULT="pass"
else
  STAGE9L_FINAL_RESULT="fail"
fi
export STAGE9L_FINAL_RESULT
write_evidence

echo
echo "=== Stage 9L smoke result ==="
if [ "$fail" = "0" ]; then
  echo "PASS: Stage 9L live-served disabled operator gate verified"
else
  echo "FAIL: Stage 9L live-served disabled operator gate found issues"
fi
