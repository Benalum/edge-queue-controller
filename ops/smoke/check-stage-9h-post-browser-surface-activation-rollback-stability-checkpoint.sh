...
REPORT="docs/generated/stage-9h-post-browser-surface-activation-rollback-stability-checkpoint.md"
EVIDENCE="docs/generated/stage-9h-post-browser-surface-activation-rollback-stability-checkpoint-evidence.json"
SMOKE="ops/smoke/check-stage-9h-post-browser-surface-activation-rollback-stability-checkpoint.sh"
...
LIVE_APP="/tmp/stage9h-live-app.js"
LIVE_STUB="/tmp/stage9h-live-router-shadow-read-stub.js"

export STAGE9H_FINAL_RESULT="unknown"
export STAGE9H_HEALTH_CODE="unknown"
export STAGE9H_POST_CODE="unknown"
export STAGE9H_ENV_ABSENT="unknown"
export STAGE9H_QUEUE_CLEAN="unknown"
...
data = {
    ...
    "purpose": "post-browser-surface activation rollback stability checkpoint",
    "health_code": os.environ.get("STAGE9H_HEALTH_CODE"),
    "post_code": os.environ.get("STAGE9H_POST_CODE"),
    "env_absent": os.environ.get("STAGE9H_ENV_ABSENT"),
    "queue_clean": os.environ.get("STAGE9H_QUEUE_CLEAN"),
    "final_result": os.environ.get("STAGE9H_FINAL_RESULT"),
}
...
echo "=== Stage 9H smoke: post-browser-surface activation rollback stability checkpoint ==="

echo
echo "=== report/script checks ==="
[ -f "$REPORT" ] && pass "Stage 9H report exists" || check_fail "Stage 9H report missing"
[ -x "$SMOKE" ] && pass "Stage 9H smoke script is executable" || check_fail "Stage 9H smoke script missing or not executable"
...
echo
echo "=== Stage 9G evidence validation ==="
if [ -f "$STAGE9G_EVIDENCE" ]; then
  pass "Stage 9G evidence exists"
else
  check_fail "Stage 9G evidence missing"
fi

python3 - "$STAGE9G_EVIDENCE" <<'PY'
import json
import sys
from pathlib import Path
...
if bad:
    print("CHECK: Stage 9G evidence did not match expected values")
    for item in bad:
        print(item)
    sys.exit(2)

print("PASS: Stage 9G evidence confirms controlled browser-surface activation and rollback")
PY
if [ "$?" = "0" ]; then
  pass "Stage 9G evidence values confirmed"
else
  check_fail "Stage 9G evidence validation failed"
fi
...
live_app_code="$(curl -sS --max-time 8 -o "$LIVE_APP" -w "%{http_code}" "$FRONTEND_BASE/app.js" 2>/tmp/stage9h-live-app.err || printf 'curl_failed')"
live_stub_code="$(curl -sS --max-time 8 -o "$LIVE_STUB" -w "%{http_code}" "$FRONTEND_BASE/router_shadow_read_stub.js" 2>/tmp/stage9h-live-stub.err || printf 'curl_failed')"
...
health_code="$(curl -sS --max-time 5 -o /tmp/stage9h-health.out -w "%{http_code}" "$BASE/health" 2>/tmp/stage9h-health.err || printf 'curl_failed')"
STAGE9H_HEALTH_CODE="$health_code"
export STAGE9H_HEALTH_CODE
...
post_code="$(curl -sS --max-time 5 -X POST \
  ...
  -d '{"text":"stage9h rollback stability check","source":"stage9h","surface":"backend-only"}' \
  -o /tmp/stage9h-router-post.out \
  -w "%{http_code}" \
  "$BASE/api/router/dry-run" 2>/tmp/stage9h-router-post.err || printf 'curl_failed')"
STAGE9H_POST_CODE="$post_code"
export STAGE9H_POST_CODE
...
if printf '%s\n' "$controller_env" | tr ' ' '\n' | grep -qx 'EDGE_UNIVERSAL_INTENT_ROUTER_DRY_RUN_ENABLED=1'; then
  STAGE9H_ENV_ABSENT="false"
  export STAGE9H_ENV_ABSENT
  check_fail "backend dry-run env is enabled"
else
  STAGE9H_ENV_ABSENT="true"
  export STAGE9H_ENV_ABSENT
...
queue_code="$(curl -sS --max-time 5 -o /tmp/stage9h-system-status.json -w "%{http_code}" "$STATUS_URL" 2>/tmp/stage9h-system-status.err || printf 'curl_failed')"
echo "queue_status_code=$queue_code"

if [ "$queue_code" = "200" ]; then
  python3 - /tmp/stage9h-system-status.json <<'PY'
import json
import sys
...
echo
echo "=== write Stage 9H evidence ==="
if [ "$fail" = "0" ]; then
  STAGE9H_FINAL_RESULT="pass"
else
  STAGE9H_FINAL_RESULT="fail"
fi
export STAGE9H_FINAL_RESULT
write_evidence

echo
echo "=== Stage 9H smoke result ==="
if [ "$fail" = "0" ]; then
  echo "PASS: Stage 9H post-browser-surface activation rollback stability verified"
else
  echo "FAIL: Stage 9H post-browser-surface activation rollback stability found issues"
fi
