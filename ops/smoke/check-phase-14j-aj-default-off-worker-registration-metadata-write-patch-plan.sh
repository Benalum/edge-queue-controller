...
echo "=== Phase 14J-AJ smoke: default-off worker registration metadata write patch plan ==="

DOC="docs/phase-14j-aj-default-off-worker-registration-metadata-write-patch-plan.md"
...
echo
echo "=== documentation plan markers ==="
python3 - <<'APC_AJ_DOC_MARKERS'
from pathlib import Path

text = Path("docs/phase-14j-aj-default-off-worker-registration-metadata-write-patch-plan.md").read_text()
...
required = [
    ...
    "`state`",
    "`computed_health`",
    "avoid making a worker lane-enabled by registration alone",
    "avoid changing scheduler selection behavior",
...
print("PASS: documentation plan markers verified")
APC_AJ_DOC_MARKERS

echo
echo "=== compile check without writing pycache ==="
python3 - <<'APC_AJ_COMPILE'
from pathlib import Path
...
echo
echo "=== future patch source markers still exist ==="
python3 - <<'APC_AJ_SOURCE_MARKERS'
from pathlib import Path
...
missing = [item for item in required if item not in text]
if missing:
    raise SystemExit("FAIL: missing future patch source markers: " + ", ".join(missing))

print("PASS: future patch source markers remain present")
APC_AJ_SOURCE_MARKERS

echo
echo "=== static guard: AJ smoke must not execute forbidden runtime actions ==="
...
echo
if [ "$fail" = "0" ]; then
  echo "PASS: Phase 14J-AJ smoke complete"
else
  echo "FAIL: Phase 14J-AJ smoke failed"
fi
