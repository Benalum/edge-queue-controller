...
echo "=== Phase 14J-V worker registry lane metadata inspection plan ==="

PHASE="phase-14j-v-worker-registry-lane-metadata-inspection-plan"
DOC="docs/${PHASE}.md"
...
echo "PASS: required 14J-V docs/smoke/plan/runtime files exist"
...
doc = Path("docs/phase-14j-v-worker-registry-lane-metadata-inspection-plan.md").read_text()
plan = Path("docs/phase-14j-v-worker-registry-lane-metadata-inspection-plan-plan.txt").read_text()
...
required_plan = [
    ...
    "worker_registry_inspection_in_this_phase:no",
    "future_inspection_goal:",
    "future_allowed_read_only_command_classes:",
    "future_expected_fields:",
...
echo
echo "=== helper metadata field references remain present ==="
python3 - <<'PY'
from pathlib import Path
...
if missing:
    raise SystemExit("FAIL: missing expected lane helper/metadata markers: " + ", ".join(missing))

print("PASS: lane helper metadata markers remain present")
PY
...
echo
echo "=== changed files limited to Phase 14J-V docs/smoke/plan ==="
python3 - <<'PY'
import subprocess
...
print("PASS: changed files are limited to Phase 14J-V docs/smoke/plan")
PY
...
smoke = Path("ops/smoke/check-phase-14j-v-worker-registry-lane-metadata-inspection-plan.sh").read_text()
...
if bad:
    raise SystemExit("FAIL: 14J-V smoke contains forbidden live/external behavior:\n" + "\n".join(bad))

print("PASS: smoke behavior guard passed")
PY

echo
echo "=== done: Phase 14J-V worker registry lane metadata inspection plan smoke complete ==="
