...
REPORT="docs/generated/stage-9g-controlled-browser-surface-activation-rollback.md"
EVIDENCE="docs/generated/stage-9g-controlled-browser-surface-activation-rollback-evidence.json"
SMOKE="ops/smoke/check-stage-9g-controlled-browser-surface-activation-rollback.sh"
...
STAGE9F_REPORT="docs/generated/stage-9f-controlled-browser-surface-activation-rollback-plan.md"
...
DROPIN="$DROPIN_DIR/86-stage9g-router-dry-run.conf"

LIVE_APP="/tmp/stage9g-live-app.js"
LIVE_STUB="/tmp/stage9g-live-router-shadow-read-stub.js"
LIVE_APP_BLOCK="/tmp/stage9g-live-app-block.js"
BROWSER_SURFACE_RESULT="/tmp/stage9g-controlled-browser-surface-result.json"

export STAGE9G_FINAL_RESULT="unknown"
export STAGE9G_HEALTH_BEFORE="unknown"
export STAGE9G_POST_BEFORE="unknown"
export STAGE9G_QUEUE_BEFORE_CLEAN="unknown"
export STAGE9G_HEALTH_ENABLED="unknown"
export STAGE9G_POST_ENABLED="unknown"
export STAGE9G_BROWSER_REQUESTS="unknown"
export STAGE9G_BROWSER_STATUS="unknown"
export STAGE9G_BROWSER_OK="unknown"
export STAGE9G_BROWSER_SURFACE="unknown"
export STAGE9G_BROWSER_DRY_RUN="unknown"
export STAGE9G_DISPATCH_REQUESTED="unknown"
export STAGE9G_DISPATCH_PERFORMED="unknown"
export STAGE9G_BODY_DISPATCH_PERFORMED="unknown"
export STAGE9G_QUEUE_ENABLED_CLEAN="unknown"
export STAGE9G_HEALTH_AFTER="unknown"
export STAGE9G_POST_AFTER="unknown"
export STAGE9G_ROLLBACK_ENV_ABSENT="unknown"
export STAGE9G_QUEUE_AFTER_CLEAN="unknown"
...
data = {
    ...
    "purpose": "controlled browser-surface activation and rollback",
    "health_before": os.environ.get("STAGE9G_HEALTH_BEFORE"),
    "post_before": os.environ.get("STAGE9G_POST_BEFORE"),
    "queue_before_clean": os.environ.get("STAGE9G_QUEUE_BEFORE_CLEAN"),
    "health_enabled": os.environ.get("STAGE9G_HEALTH_ENABLED"),
    "post_enabled": os.environ.get("STAGE9G_POST_ENABLED"),
    "browser_requests": os.environ.get("STAGE9G_BROWSER_REQUESTS"),
    "browser_status": os.environ.get("STAGE9G_BROWSER_STATUS"),
    "browser_ok": os.environ.get("STAGE9G_BROWSER_OK"),
    "browser_surface": os.environ.get("STAGE9G_BROWSER_SURFACE"),
    "browser_dry_run": os.environ.get("STAGE9G_BROWSER_DRY_RUN"),
    "dispatch_requested": os.environ.get("STAGE9G_DISPATCH_REQUESTED"),
    "dispatch_performed": os.environ.get("STAGE9G_DISPATCH_PERFORMED"),
    "body_dispatch_performed": os.environ.get("STAGE9G_BODY_DISPATCH_PERFORMED"),
    "queue_enabled_clean": os.environ.get("STAGE9G_QUEUE_ENABLED_CLEAN"),
    "health_after": os.environ.get("STAGE9G_HEALTH_AFTER"),
    "post_after": os.environ.get("STAGE9G_POST_AFTER"),
    "rollback_env_absent": os.environ.get("STAGE9G_ROLLBACK_ENV_ABSENT"),
    "queue_after_clean": os.environ.get("STAGE9G_QUEUE_AFTER_CLEAN"),
    "final_result": os.environ.get("STAGE9G_FINAL_RESULT"),
}
...
wait_for_health() {
  ...
  code="000"
  while [ "$tries" -gt 0 ]; do
    code="$(curl -sS --max-time 3 -o "/tmp/stage9g-health-${label}.out" -w "%{http_code}" "$BASE/health" 2>"/tmp/stage9g-health-${label}.err" || printf 'curl_failed')"
    if [ "$code" = "200" ]; then
      echo "$code"
      return 0
    fi
...
queue_clean_check() {
  label="$1"
  out="/tmp/stage9g-system-status-${label}.json"
  err="/tmp/stage9g-system-status-${label}.err"
...
rollback_backend() {
  if [ "$rollback_needed" = "1" ]; then
    echo
    echo "=== rollback: remove Stage 9G backend dry-run drop-in and restart controller ==="
    ...
    STAGE9G_HEALTH_AFTER="$health_after"
    export STAGE9G_HEALTH_AFTER
    ...
    if printf '%s\n' "$controller_env_after" | tr ' ' '\n' | grep -qx 'EDGE_UNIVERSAL_INTENT_ROUTER_DRY_RUN_ENABLED=1'; then
      STAGE9G_ROLLBACK_ENV_ABSENT="false"
      export STAGE9G_ROLLBACK_ENV_ABSENT
      check_fail "rollback failed: backend dry-run env still enabled"
    else
      STAGE9G_ROLLBACK_ENV_ABSENT="true"
      export STAGE9G_ROLLBACK_ENV_ABSENT
    ...
    post_after="$(curl -sS --max-time 5 -X POST \
      ...
      -d '{"text":"stage9g rollback check","source":"stage9g","surface":"backend-only"}' \
      -o /tmp/stage9g-router-post-after.out \
      -w "%{http_code}" \
      "$BASE/api/router/dry-run" 2>/tmp/stage9g-router-post-after.err || printf 'curl_failed')"
    STAGE9G_POST_AFTER="$post_after"
    export STAGE9G_POST_AFTER
    ...
    if queue_clean_check after; then
      STAGE9G_QUEUE_AFTER_CLEAN="true"
      export STAGE9G_QUEUE_AFTER_CLEAN
      pass "queue clean after rollback"
    else
      STAGE9G_QUEUE_AFTER_CLEAN="false"
      export STAGE9G_QUEUE_AFTER_CLEAN
...
trap 'rollback_backend; STAGE9G_FINAL_RESULT="fail"; export STAGE9G_FINAL_RESULT; write_evidence' INT TERM

echo "=== Stage 9G smoke: controlled browser-surface activation and rollback ==="

echo
echo "=== report/script checks ==="
[ -f "$REPORT" ] && pass "Stage 9G report exists" || check_fail "Stage 9G report missing"
[ -x "$SMOKE" ] && pass "Stage 9G smoke script is executable" || check_fail "Stage 9G smoke script missing or not executable"
...
echo
echo "=== repo cleanliness check allowing only Stage 9G generated files ==="
dirty="$(git status --short)"
unexpected="$(printf '%s\n' "$dirty" | grep -vE '^\?\? docs/generated/stage-9g-controlled-browser-surface-activation-rollback(\.md|-evidence\.json)$|^\?\? ops/smoke/check-stage-9g-controlled-browser-surface-activation-rollback\.sh$|^$' || true)"
...
echo
echo "=== Stage 9D/9E/9F evidence and plan checks ==="
...
[ -f "$STAGE9F_REPORT" ] && pass "Stage 9F report exists" || check_fail "Stage 9F report missing"
...
live_app_code="$(curl -sS --max-time 8 -o "$LIVE_APP" -w "%{http_code}" "$FRONTEND_BASE/app.js" 2>/tmp/stage9g-live-app.err || printf 'curl_failed')"
live_stub_code="$(curl -sS --max-time 8 -o "$LIVE_STUB" -w "%{http_code}" "$FRONTEND_BASE/router_shadow_read_stub.js" 2>/tmp/stage9g-live-stub.err || printf 'curl_failed')"
...
echo
echo "=== extract live-served Stage 9D bridge block ==="
...
print(f"wrote live app block: {out}")
PY
if [ "$?" = "0" ]; then
  pass "live-served Stage 9D bridge block extracted"
else
  check_fail "could not extract live-served Stage 9D bridge block"
fi

echo
echo "=== pre-activation live backend checks ==="
health_before="$(wait_for_health before || true)"
STAGE9G_HEALTH_BEFORE="$health_before"
export STAGE9G_HEALTH_BEFORE
...
post_before="$(curl -sS --max-time 5 -X POST \
  -H 'Content-Type: application/json' \
  -d '{"text":"stage9g pre activation check","source":"stage9g","surface":"backend-only"}' \
  -o /tmp/stage9g-router-post-before.out \
  -w "%{http_code}" \
  "$BASE/api/router/dry-run" 2>/tmp/stage9g-router-post-before.err || printf 'curl_failed')"
STAGE9G_POST_BEFORE="$post_before"
export STAGE9G_POST_BEFORE
...
if queue_clean_check before; then
  STAGE9G_QUEUE_BEFORE_CLEAN="true"
  export STAGE9G_QUEUE_BEFORE_CLEAN
  pass "queue clean before activation"
else
  STAGE9G_QUEUE_BEFORE_CLEAN="false"
  export STAGE9G_QUEUE_BEFORE_CLEAN
...
EOF
  ...
  STAGE9G_HEALTH_ENABLED="$health_enabled"
  export STAGE9G_HEALTH_ENABLED
  ...
  post_enabled="$(curl -sS --max-time 8 -X POST \
    -H 'Content-Type: application/json' \
    -d '{"text":"stage9g backend enabled check","source":"stage9g","surface":"backend-only"}' \
    -o /tmp/stage9g-router-post-enabled.out \
    -w "%{http_code}" \
    "$BASE/api/router/dry-run" 2>/tmp/stage9g-router-post-enabled.err || printf 'curl_failed')"
  STAGE9G_POST_ENABLED="$post_enabled"
  export STAGE9G_POST_ENABLED
  ...
  echo
  echo "=== exactly one controlled browser-surface request through live-served bridge ==="
  if command -v node >/dev/null 2>&1 && [ -f "$LIVE_STUB" ] && [ -f "$LIVE_APP_BLOCK" ]; then
    STAGE9G_BACKEND_URL="$BASE/api/router/dry-run" \
    STAGE9G_RESULT_FILE="$BROWSER_SURFACE_RESULT" \
    node <<'NODE'
const fs = require("fs");
const vm = require("vm");

const stub = fs.readFileSync("/tmp/stage9g-live-router-shadow-read-stub.js", "utf8");
const appBlock = fs.readFileSync("/tmp/stage9g-live-app-block.js", "utf8");
const backendUrl = process.env.STAGE9G_BACKEND_URL;
const resultFile = process.env.STAGE9G_RESULT_FILE;
...
const sandbox = {
  ...
  navigator: { userAgent: "stage9g-controlled-browser-surface" },
  location: { href: "http://127.0.0.1:8787/stage9g-controlled-browser-surface" },
  localStorage: {
    getItem: () => null,
    setItem: () => {},
    removeItem: () => {}
  ...
  fetch: async (url, options) => {
    fetchCalls += 1;
    if (fetchCalls > 1) {
      throw new Error("more than one browser-surface fetch attempted");
    }

    const target = String(url);
    if (target !== "/api/router/dry-run") {
      throw new Error("unexpected browser-surface endpoint: " + target);
    }
    ...
    if (parsed.dry_run !== true) {
      throw new Error("browser-surface request did not set dry_run=true");
    }
    if (parsed.dispatch_requested !== false || parsed.dispatch_performed !== false) {
      throw new Error("browser-surface request was not explicitly non-dispatching");
    }
...
vm.runInContext(stub, sandbox, { filename: "stage9g-live-router-shadow-read-stub.js" });
vm.runInContext(appBlock, sandbox, { filename: "stage9g-live-app-block.js" });
...
if (!surface) {
  throw new Error("EdgeRouterShadowReadSurface namespace missing");
}
...
Promise.resolve(
  surface.requestBrowserSurfaceRouterShadowRead("manual-diagnostic", {
    text: "stage9g exactly one controlled browser-surface shadow-read",
    source: "stage9g",
    route_hint: "controlled-browser-surface"
  })
).then((result) => {
  ...
  if (fetchCalls !== 1) {
    throw new Error("expected exactly one browser-surface fetch, got " + fetchCalls);
  }
  if (!output.ok || output.status !== 200) {
    throw new Error("controlled browser-surface shadow-read did not return HTTP 200");
  }
  ...
  console.log("PASS: controlled browser-surface shadow-read sent exactly one non-dispatching request");
}).catch((error) => {
  try {
    shadow.ROUTER_SHADOW_READ_ENABLED = false;
    shadow.ROUTER_SHADOW_READ_FEATURE_FLAG_DEFAULT = false;
...
NODE
    browser_node_check=$?
    if [ "$browser_node_check" = "0" ]; then
      pass "controlled browser-surface Node VM check passed"
    else
      check_fail "controlled browser-surface Node VM check failed"
    fi
  else
    check_fail "node or live-served files unavailable for controlled browser-surface request"
  fi

  if [ -f "$BROWSER_SURFACE_RESULT" ]; then
    cat "$BROWSER_SURFACE_RESULT"

    browser_requests="$(python3 -c 'import json; print(json.load(open("'"$BROWSER_SURFACE_RESULT"'")).get("requests"))' 2>/dev/null || echo unknown)"
    browser_status="$(python3 -c 'import json; print(json.load(open("'"$BROWSER_SURFACE_RESULT"'")).get("status"))' 2>/dev/null || echo unknown)"
    browser_ok="$(python3 -c 'import json; print(json.load(open("'"$BROWSER_SURFACE_RESULT"'")).get("ok"))' 2>/dev/null || echo unknown)"
    browser_surface="$(python3 -c 'import json; print(json.load(open("'"$BROWSER_SURFACE_RESULT"'")).get("observed_surface"))' 2>/dev/null || echo unknown)"
    browser_dry_run="$(python3 -c 'import json; print(json.load(open("'"$BROWSER_SURFACE_RESULT"'")).get("observed_dry_run"))' 2>/dev/null || echo unknown)"
    dispatch_requested="$(python3 -c 'import json; print(json.load(open("'"$BROWSER_SURFACE_RESULT"'")).get("observed_dispatch_requested"))' 2>/dev/null || echo unknown)"
    dispatch_performed="$(python3 -c 'import json; print(json.load(open("'"$BROWSER_SURFACE_RESULT"'")).get("observed_dispatch_performed"))' 2>/dev/null || echo unknown)"
    body_dispatch_performed="$(python3 -c 'import json; print(json.load(open("'"$BROWSER_SURFACE_RESULT"'")).get("body_dispatch_performed"))' 2>/dev/null || echo unknown)"

    STAGE9G_BROWSER_REQUESTS="$browser_requests"
    STAGE9G_BROWSER_STATUS="$browser_status"
    STAGE9G_BROWSER_OK="$browser_ok"
    STAGE9G_BROWSER_SURFACE="$browser_surface"
    STAGE9G_BROWSER_DRY_RUN="$browser_dry_run"
    STAGE9G_DISPATCH_REQUESTED="$dispatch_requested"
    STAGE9G_DISPATCH_PERFORMED="$dispatch_performed"
    STAGE9G_BODY_DISPATCH_PERFORMED="$body_dispatch_performed"
    export STAGE9G_BROWSER_REQUESTS STAGE9G_BROWSER_STATUS STAGE9G_BROWSER_OK
    export STAGE9G_BROWSER_SURFACE STAGE9G_BROWSER_DRY_RUN STAGE9G_DISPATCH_REQUESTED
    export STAGE9G_DISPATCH_PERFORMED STAGE9G_BODY_DISPATCH_PERFORMED

    [ "$browser_requests" = "1" ] && pass "exactly one browser-surface request was sent" || check_fail "browser-surface request count was not exactly one"
    [ "$browser_status" = "200" ] && pass "browser-surface shadow-read returned HTTP 200" || check_fail "browser-surface shadow-read did not return HTTP 200"
    case "$browser_ok" in True|true) pass "browser-surface shadow-read ok=true" ;; *) check_fail "browser-surface shadow-read ok was not true" ;; esac
    ...
    case "$browser_dry_run" in True|true) pass "browser-surface dry_run=true" ;; *) check_fail "browser-surface dry_run was not true" ;; esac
    case "$dispatch_requested" in False|false) pass "browser-surface dispatch_requested=false" ;; *) check_fail "browser-surface dispatch_requested was not false" ;; esac
    case "$dispatch_performed" in False|false) pass "browser-surface dispatch_performed=false" ;; *) check_fail "browser-surface dispatch_performed was not false" ;; esac
    ...
    check_fail "browser-surface result file missing"
  fi

  if queue_clean_check enabled; then
    STAGE9G_QUEUE_ENABLED_CLEAN="true"
    export STAGE9G_QUEUE_ENABLED_CLEAN
    pass "queue remained clean after controlled browser-surface request"
  else
    STAGE9G_QUEUE_ENABLED_CLEAN="false"
    export STAGE9G_QUEUE_ENABLED_CLEAN
    check_fail "queue not clean after controlled browser-surface request"
  fi
fi
...
grep -q "requestBrowserSurfaceRouterShadowRead" "$APP_JS" 2>/dev/null \
  && pass "app.js still contains requestBrowserSurfaceRouterShadowRead after rollback" \
  || check_fail "app.js missing requestBrowserSurfaceRouterShadowRead after rollback"
...
echo
echo "=== write Stage 9G evidence ==="
if [ "$fail" = "0" ]; then
  STAGE9G_FINAL_RESULT="pass"
else
  STAGE9G_FINAL_RESULT="fail"
fi
export STAGE9G_FINAL_RESULT
write_evidence

echo
echo "=== Stage 9G smoke result ==="
if [ "$fail" = "0" ]; then
  echo "PASS: Stage 9G controlled browser-surface activation and rollback verified"
else
  echo "FAIL: Stage 9G controlled browser-surface activation and rollback found issues"
fi
