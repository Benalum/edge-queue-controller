...
DOC="docs/stage-17k-r14s-load-disabled-save-button-render-spec-no-ui-no-binding.md"
OUT_DIR="docs/smoke/generated/stage-17k-r14s-load-disabled-save-button-render-spec-no-ui-no-binding"
...
grep -Fq "Load Disabled Save Button Render Spec, No UI/Binding" "$DOC"
...
grep -Fq "stage17k-r14s-load-disabled-save-button-render-spec-no-ui-no-binding-20260705" "$INDEX"
...
if grep -Fq "APC_LOCAL_BACKUP_CURRENT_FILE_DISABLED_SAVE_BUTTON_RENDER_SPEC" "$MOUNT" "$PANEL"; then
  echo "FAIL: mount/panel must not reference render spec"
  exit 1
fi

grep -Fq "PASS exact script delta old+1 with no removals" "$OUT_DIR/source-check."*.txt
grep -Fq "R14S_VM200_LOAD_DISABLED_SAVE_BUTTON_RENDER_SPEC_NO_UI_NO_BINDING_DEPLOY_DONE" "$OUT_DIR/vm200-deploy."*.txt
grep -Fq "PASS public static R14S smoke" "$OUT_DIR/public-static-smoke."*.txt
...
echo "PASS stage-17k-r14s load disabled save button render spec no ui no binding smoke"
