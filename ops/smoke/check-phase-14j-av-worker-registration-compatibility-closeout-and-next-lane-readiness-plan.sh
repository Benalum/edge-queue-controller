...
echo "=== Phase 14J-AV smoke: worker registration compatibility closeout and next-lane-readiness plan ==="

DOC="docs/phase-14j-av-worker-registration-compatibility-closeout-and-next-lane-readiness-plan.md"
...
for p in \
  ...
  "ops/smoke/check-phase-14j-au-post-reload-compatibility-result-checkpoint.sh" \
...
python3 - <<'APC_AV_DOC_CHECK'
from pathlib import Path

text = Path("docs/phase-14j-av-worker-registration-compatibility-closeout-and-next-lane-readiness-plan.md").read_text()
...
required = [
    ...
    "A successful reload is not lane dispatch activation.",
...
echo
echo "=== source remains docs/smoke only for AV ==="
if git diff --name-only | grep -q '^edge_controller.py$'; then
  echo "FAIL: edge_controller.py changed during AV"
  fail=1
else
  echo "PASS: edge_controller.py unchanged in AV"
fi

echo
echo "=== default-off DB and env state ==="
...
health_code="$(
  curl -sS --max-time 5 -o /tmp/phase14j-av-local-health.json \
    -w "%{http_code}" \
    http://127.0.0.1:7070/system/local-health 2>/tmp/phase14j-av-local-health.err || true
)"
...
echo
echo "=== static guard: AV smoke must not execute forbidden runtime actions ==="
...
if [ -n "$forbidden_runtime_lines" ]; then
  echo "FAIL: AV smoke contains executable forbidden runtime lines"
  echo "$forbidden_runtime_lines"
  fail=1
else
  echo "PASS: AV smoke contains no executable forbidden runtime lines"
fi

echo
if [ "$fail" = "0" ]; then
  echo "PASS: Phase 14J-AV smoke complete"
else
  echo "FAIL: Phase 14J-AV smoke failed"
fi
