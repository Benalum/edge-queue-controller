...
try:
    ...
    job = conn.execute("select id, status, attempts from jobs where id = 25").fetchone()
    print("job_25_status=" + str(job["status"]))
    print("job_25_attempts=" + str(job["attempts"]))
    print("job_results_for_job_25=" + str(conn.execute("select count(*) from job_results where job_id = 25").fetchone()[0]))
    print("job_results_marker_matches_for_job_25=" + str(conn.execute(
        "select count(*) from job_results where job_id = 25 and coalesce(response_json, '') like ?",
        (f"%{RESULT_MARKER}%",),
    ).fetchone()[0]))
finally:
    conn.close()
...
{
echo "=== Stage 16 E3M-A smoke: manual completion helper no-run ==="
...
echo
echo "--- helper and adapter static checks ---"
...
"$HELPER" --help | grep -q "Manual queued-job completion helper" || mark_fail "helper help missing title"
grep -q "APPROVE_STAGE_16_E3M_B_RUN_MANUAL_COMPLETION_HELPER" "$HELPER" || mark_fail "helper missing E3M-B approval gate"
grep -q "APC_ONE_SHOT_MODEL_APPROVAL" "$HELPER" || mark_fail "helper missing adapter approval environment"
grep -q "insert into job_results" "$HELPER" || mark_fail "helper missing job_results insert"
grep -q "update jobs" "$HELPER" || mark_fail "helper missing jobs update"
grep -q "status = 'queued'" "$HELPER" || mark_fail "helper missing queued status guard"
grep -q "result already exists" "$HELPER" || mark_fail "helper missing idempotency guard"

echo
echo "--- helper negative approval-gate check: should not run DB/model ---"
set +e
gate_out="$(JOB_ID=25 "$HELPER" 2>&1)"
gate_rc="$?"
set -u
echo "helper_no_approval_rc=$gate_rc"
printf '%s\n' "$gate_out" | head -30
[ "$gate_rc" = "64" ] || mark_fail "helper did not stop at approval gate rc=64"
printf '%s\n' "$gate_out" | grep -q "approval_gate=FAIL" || mark_fail "helper approval failure text missing"
...
for spec in "/:200" "/login:200" "/api/me:401" "/api/system/status:200" "/system/status:200"; do
  path="${spec%:*}"
  expect="${spec##*:}"
  code="$(curl -sS -o /tmp/apc_e3m_a_smoke_public_body.txt -w '%{http_code}' --max-time 20 "${DOMAIN}${path}" 2>/tmp/apc_e3m_a_smoke_public_err.txt || true)"
  echo "public_path=$path http=${code:-<curl_failed>} expect=$expect"
  [ "$code" = "$expect" ] || mark_fail "public route mismatch: $path"
done
rm -f /tmp/apc_e3m_a_smoke_public_body.txt /tmp/apc_e3m_a_smoke_public_err.txt
