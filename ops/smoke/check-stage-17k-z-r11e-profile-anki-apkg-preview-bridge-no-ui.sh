...
if grep -n "profile-anki-import-bridge.js" frontend/wrapper-ui/apc-wrapper-local/index.html frontend/wrapper-ui/apc-wrapper-local/privatepages/pages/profile.html >/tmp/apc-r11e-mounted-check.txt 2>/dev/null; then
  echo "FAIL: R11E bridge must not be mounted in UI yet"
