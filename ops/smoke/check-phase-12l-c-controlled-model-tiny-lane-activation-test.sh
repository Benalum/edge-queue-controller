...
TEST_JOB_ID="phase12l-tiny-job-7ddc80a044438855"

echo "=== Phase 12L-C smoke: controlled model-tiny lane activation test ==="
...
echo
echo "=== doc checks ==="
DOC="docs/phase-12l-c-controlled-model-tiny-lane-activation-test.md"
grep -Fq "Phase 12L-C Controlled Model-Tiny Lane Activation Test" "$DOC" && echo "PASS: doc title found" || fail=1
grep -Fq "$TEST_JOB_ID" "$DOC" && echo "PASS: test job id documented" || fail=1
grep -Fq "ct101-stage5g21-managed-browser-model-tiny" "$DOC" && echo "PASS: tiny worker documented" || fail=1
grep -Fq "tiny lane ok" "$DOC" && echo "PASS: tiny reply documented" || fail=1
grep -Fq "does not yet enable persistent lane workers" "$DOC" && echo "PASS: limitation documented" || fail=1

echo
echo "=== controller health ==="
curl -sS --max-time 8 -o /tmp/phase12lc-smoke-health.json -w "health_code=%{http_code} time=%{time_total}\n" http://127.0.0.1:7070/health || fail=1
python3 -m json.tool /tmp/phase12lc-smoke-health.json >/tmp/phase12lc-smoke-health.pretty && grep -Fq '"ok": true' /tmp/phase12lc-smoke-health.pretty && echo "PASS: controller health ok" || fail=1

echo
echo "=== controlled test job proof ==="
PHASE12LC_TEST_JOB_ID="$TEST_JOB_ID" python3 - <<'PY' || fail=1
import json
import os
import subprocess

job_id = os.environ["PHASE12LC_TEST_JOB_ID"]
sql = f"""
...
assert rows, "controlled tiny test job not found"
row = rows[0]
...
assert row["assigned_worker_id"] == "ct101-stage5g21-managed-browser-model-tiny", row
...
assert result.get("model") == "qwen3:0.6b", result
assert result.get("worker") == "ct101-stage5g21-managed-browser-model-tiny", result
assert result.get("reply") == "tiny lane ok", result

print("PASS: controlled model-tiny job proof ok")
PY

echo
echo "=== CT101 service safety state ==="
ssh "$CT101" 'pct exec 101 -- systemctl is-active ai-platform-laptop-queue-worker.service' >/tmp/phase12lc-primary-active.txt && grep -Fq "active" /tmp/phase12lc-primary-active.txt && echo "PASS: primary worker active" || fail=1

if ssh "$CT101" 'pct exec 101 -- systemctl is-active ai-platform-laptop-queue-worker@model-tiny.service' >/tmp/phase12lc-tiny-active.txt 2>/dev/null; then
  echo "FAIL: tiny lane service active unexpectedly"
  cat /tmp/phase12lc-tiny-active.txt
  fail=1
else
  echo "PASS: tiny lane service inactive"
fi

if ssh "$CT101" 'pct exec 101 -- systemctl is-active ai-platform-laptop-queue-worker@model-small.service' >/tmp/phase12lc-small-active.txt 2>/dev/null; then
  echo "FAIL: small lane service active unexpectedly"
  cat /tmp/phase12lc-small-active.txt
  fail=1
else
...
echo
echo "=== CT101 source-safe tiny env remains ready ==="
ssh "$CT101" 'pct exec 101 -- grep -nE "^(LAPTOP_QUEUE_WORKER_ID|LAPTOP_QUEUE_WORKER_NODE_ID|LAPTOP_QUEUE_WORKER_NAME|LAPTOP_QUEUE_WORKER_NODE_NAME|LAPTOP_QUEUE_QUEUE_LANE|LAPTOP_QUEUE_ALLOWED_MODELS|LAPTOP_QUEUE_OLLAMA_MODEL_FALLBACK|LAPTOP_QUEUE_MAX_JOBS_PER_RUN|LAPTOP_QUEUE_NODE_MAX_CONCURRENT_JOBS)=" /etc/ai-platform/laptop-queue-worker-model-tiny.env' >/tmp/phase12lc-tiny-env.txt && echo "PASS: tiny env captured" || fail=1
cat /tmp/phase12lc-tiny-env.txt

grep -Fq "LAPTOP_QUEUE_WORKER_ID=ct101-stage5g21-managed-browser-model-tiny" /tmp/phase12lc-tiny-env.txt && echo "PASS: tiny worker id ready" || fail=1
grep -Fq "LAPTOP_QUEUE_QUEUE_LANE=model-tiny" /tmp/phase12lc-tiny-env.txt && echo "PASS: tiny queue lane ready" || fail=1
grep -Fq "LAPTOP_QUEUE_ALLOWED_MODELS=qwen3:0.6b" /tmp/phase12lc-tiny-env.txt && echo "PASS: tiny allowed model ready" || fail=1
grep -Fq "LAPTOP_QUEUE_WORKER_NAME=CT101_model_tiny_lane_worker" /tmp/phase12lc-tiny-env.txt && echo "PASS: tiny name source-safe" || fail=1

echo
echo "=== controller live status remains primary-only after controlled test ==="
curl -sS --max-time 12 http://127.0.0.1:7070/system/status | python3 -m json.tool >/tmp/phase12lc-system-status.json || fail=1

python3 - <<'PY' || fail=1
import json

data = json.load(open("/tmp/phase12lc-system-status.json"))
...
bad_status="$(git status --short | grep -vF '?? docs/phase-12l-c-controlled-model-tiny-lane-activation-test.md' | grep -vF '?? ops/smoke/check-phase-12l-c-controlled-model-tiny-lane-activation-test.sh' || true)"
git status --short
...
if [ -n "$bad_status" ]; then
  ...
  echo "PASS: only Phase 12L-C doc/smoke files changed"
fi

echo
if [ "$fail" = "0" ]; then
  echo "PASS: Phase 12L-C controlled model-tiny lane activation smoke passed"
else
  echo "FAIL: Phase 12L-C controlled model-tiny lane activation smoke failed"
fi
