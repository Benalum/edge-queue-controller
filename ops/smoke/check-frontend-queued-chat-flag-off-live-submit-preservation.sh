...
echo "=== frontend queued chat flag-off live submit preservation smoke ==="
...
require_fixed docs/frontend-queued-chat-flag-off-live-submit-preservation.md "static verification only" "static only"
require_fixed docs/frontend-queued-chat-flag-off-live-submit-preservation.md "does not change frontend runtime behavior" "no runtime change"
require_fixed docs/frontend-queued-chat-flag-off-live-submit-preservation.md "does not wire queued chat into the real submit handler" "no submit wiring"
require_fixed docs/frontend-queued-chat-flag-off-live-submit-preservation.md "does not enable queued chat by default" "default disabled"
require_fixed docs/frontend-queued-chat-flag-off-live-submit-preservation.md "current legacy submit path remains active" "legacy submit preserved"
require_fixed docs/frontend-queued-chat-flag-off-live-submit-preservation.md "Stage 5F-55 should add a mocked flag-on submit wiring plan" "next stage"
...
# Strip isolated Stage 5F helper branches. These are direct-test helpers and
# must not be confused with live submit behavior.
helper_names = [
...
if marker_line is None:
    print("FAIL: marker line could not be located")
    sys.exit(1)

nearby = "\n".join(lines[marker_line - 1:min(len(lines), marker_line + 90)])
if not any(anchor in nearby for anchor in anchors):
    print("FAIL: Stage 5F-43 marker is no longer near a submit/send anchor")
    print(f"marker_line={marker_line}")
    sys.exit(1)

print("OK: flag-off live submit path has no queued helper calls")
print("OK: Stage 5F-43 marker remains near live submit path")
PYCHECK
...
echo "PASS: frontend queued chat flag-off live submit preservation smoke passed"
